...
- The VtsScore power form has a dead-troops helper. Members whose troops died tick "I have dead troops to count" and an easy-fill grid appears: one row per troop type (Cavalry, Footmen, Archers), one column per tier (Lofty (T11), T10 Enhanced, T10, T9 Enhanced, T9), entered in thousands or millions. Each field shows both numbers — the real troop count and the power it returns — at 8.2 per Lofty troop, 7.5 for T10 and 7.0 for T9, with Enhanced matching its tier, plus +0.5/+1/+5/+10/+20 quick-fill chips for the field in focus. The total is added on top of the entered Troop Power and Total Combat Power when the breakdown is saved, and the form works the same on phones and desktop (light theme included). Rows show tier badges and have an icon slot that fills in when the in-game unit images are added to assets/troops/.
...
- Velo knows the whole site now. Its toolkit map adds VtsScore / Competition #12, the Buildings planner, Eden Pathing, the Eden Operations Lab, the hub PDF tabs, the current Eden X2 season page and the Issue or Complaint form (the page every footer's "Contact Devs" link opens), and it explains that the X8 catch-up lineups only appear for players who own X8 heroes. A unit test fails if any of these is missing or if any link points at a page, tab or sub-tab that does not exist.
- New read-only answers: Competition #12 phase, next deadline and every phase's open and close time, in game time (UTC−2) and your own time, plus the BoH and Epic slot times; Castle 26–30 resource costs and any building's 26–30 Orichalcum and prerequisites, with blank sheet cells left unknown; how many pathers a route needs (40 tiles per pather, rounded up); and the Operations Lab's shared staffing counters.
- **My Competition #12.** With a new "My Competition #12" permission in Velo's privacy settings, a signed-in member can ask about their own registration: which power fields are filled, ROC level, chosen BoH and Epic times, and their own baseline and growth once the board is published. It reads only the member's own documents and the public board, and a signed-out visitor is told to sign in on VtsScore.
- When the drawer opens and a Competition #12 registration, final check or re-upload closes within 48 hours, a small line under the header says so, for example "Competition #12 re-upload closes in 5h · game 22:00 (UTC−2) · your time 00:00". Tap it to ask Velo, or dismiss it for that deadline. Answers from the new tools end with buttons such as Open VtsScore, Open Buildings planner, Open Eden Pathing, Open Operations Lab and Open the complaint form, all translated in 13 languages. **Action for the owner:** these tools need `npm run worker:deploy` as well; no Firestore rules deploy is needed.
...
- Competition #12 now uses the correct game time. Game time is UTC−2 (06:00 in Dubai is 00:00 game time, the same clock as the site header), but the competition schedule, the VTS Admin schedule panel and the member page's phase countdown were using UTC+2, so every competition time was 4 hours off. All of them now read the game-time offset from the site clock, and the admin help text says UTC−2 in every language. **Action for superadmins:** a schedule saved before this release is 4 hours early. After this release is live, open VTS Admin → 2027 Signups, check every time in the Competition #12 schedule (or refill it), and press Save schedule again. The scheduled registration job needs no redeploy, because it compares absolute times.
- The Competition #12 schedule panel can fill a 2-week default. Pick a start date (the next game day by default) and press **Fill 2-week default**: registration runs days 1–2, the final check until the end of day 4, the growth re-upload on days 13–14 and the winners' display on days 15–16. Every phase opens at 00:00 and closes at 22:00 game time. When no schedule is saved yet the form starts with this default, and nothing is saved until you press Save schedule; the phase preview keeps saying no schedule is saved until then.
...
- Heroes & Combos, Research & Towers, Class Development and Eden each have a **PDFs** tab that builds a printable document from your choices (season, troop, focus, paper size, level of detail), with numbered sections, repeated table headers and page numbers. Unknown values print as Unknown, never 0. Research & Towers also gained a **Buildings** sub-tab, and More ▸ Buildings opens it.
- Eden Siege fun pass: five tower tiers, armoured, swift and shielded enemies, warlord boss waves, Velo's dash and ultimate, a training wave, endless mode and a Daily Siege with the same seed for everyone. Runs end on a results screen with stars and recent runs saved in the browser, and touch players can call the next wave early. Arcade features today's Daily Siege, and the homepage shows a dismissible invitation.
...
- Fixed: setting a Specialization Towers research to 100% now buys both upgrades on the base-attribute nodes instead of stopping at 11/12. The combo PNG's "Generated by" and date lines no longer overlap the branding footer.
...
- One look across the standalone tools: VtsScore, Downloads, Arcade, Eden, Eden Siege, Specialization Towers and Battle Simulator share the same footer and link set, and VtsScore and Downloads gained the branded bar with Back to tools. Eden's deck button now reads Back to Tools, and the account button that was clipped in the Arcade and Eden decks has its own place beside the back link.
...
- Specialization Towers: every node now shows its medal cost from the community workbook for all three troops (735 nodes each), where it used to say the cost was unknown. Repeated node names at different strengths are matched by the strength the workbook states, and five naming differences between the workbook and the planner are recorded rather than rewritten. The header wraps instead of letting the toolbar cover the page title, and it carries the site logo and name.
- Eden Operations Lab: a task board with six common jobs filtered by season stage and role, a saved three-step checklist per job, one-tap Critical/Essential/Advanced specialty routes, attacker and support counters for the chosen siege objective, links into Eden Map and Loyalty, and the data author's credit on the source cards. Eden Pathing is announced as coming soon, and the Eden Hub sub-tabs are readable in the light theme.
...
- Buildings: added Castle 26–30 direct costs across Orichalcum, gold, food, lumber, charcoal, marble and iron, plus a searchable planner for the 43 buildings and their level costs, prerequisites and final-level bonuses. Missing sheet cells remain unknown, and the sheet's mismatched aggregate total is called out.
...
- Community Downloads: a downloads page offers English PDFs of every data breakdown — research costs, Specialisation medals, Eden tables, Dragon Master costs and the hero roster by season, kept out of the offline cache so a visit never downloads them unasked.
- Eden 2027 rewards: the reward distribution is a per-season superadmin setting — four support, ten contribution, three management and three team players by default — and the published page, the announcement card and its "Top N" follow it. The R5 always holds the guild-master reward outside the support quota, so support work rewards its full quota from everyone else.
- Eden 2027 scoring: two season multipliers — in-game contribution, and support work, which scales shield walls, pathing, banners and the form's bonus points together — and every breakdown line shows the points it adds, so the lines always sum to the score. Linked accounts gained a third "secondary" class scored at the alt weight by default, teachable nicknames (zubs → Lady Zubbs, kiji → MalakaKiji), a "link all suggestions" action, and an Accounts tab.
- Eden 2027 voting and pages: the members' and the R4/R5 results have separate public switches that work independently; duty counts open to show what each account class did; long admin tables start at ten rows; members can sign up for the season from the page, My Stats opens on their own row and the vote name fills in when their signed-in game name matches exactly.
- The 2027 registration form is back on VtsScore with a superadmin season and scoring-version picker; leadership can add and correct hand-filed signups, never overwrite a member's own.
- Issue or Complaint: members can file a bug, conduct, fair-play, alliance, missing-feature or other report with up to three private screenshots, named or anonymous; only superadmins can read or delete them, and filings are rate-limited.
- Seasons: a superadmin registry records every season. The running season can be registered, renamed, ended (a snapshot downloads first and the workspace freezes in the same write) and browsed afterwards; a snapshot can be recalled behind a dry run that writes only what changed.
