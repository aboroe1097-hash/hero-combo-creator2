...
- **VTS Player Alias Reconciliation**: Folded decoration, whitespace and case variants of the same account into one identity across leaderboards, contribution matching and weighted scoring, adopting Moldo1313 and D O F F Y as display names. Just as importantly, separated accounts that were previously being merged into their owner — DvD18 / DvD18 x2, BOiiE / BOiiE BANNER, ANGEL / Angel Banner, and the Kika main, alt and banner accounts now score independently. Candidate merges whose letters differ (FALLEN, Made3110, Dobby, пупОк and eleven others) are recorded as pending and deliberately do not resolve until confirmed, since a wrong merge silently moves one account's demolition points into another's score. Historical attack rows are unchanged.
