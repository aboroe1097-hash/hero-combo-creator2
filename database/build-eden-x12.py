#!/usr/bin/env python3
"""Merge database/eden-wonders-screenshots-x12-backup/*.json into Wonder X12 txt, then rebuild JS.

Workflow:
  python database/build-eden-x12.py
"""
...
DB = ROOT / "database"
JSON_DIR = DB / "eden-wonders-screenshots-x12-backup"
OUT_TXT = DB / "Eden_Wonders_NorthvSouth_X12_Map.txt"
BUILD_DATASETS = DB / "build-eden-datasets.py"
...
def main() -> None:
    rows = load_rows()
    if not rows:
        raise SystemExit("No X12 rows found — check database/eden-wonders-screenshots-x12-backup/")
