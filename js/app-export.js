...
async function renderCombosToCanvas(combosData, title) {
  const exportCopy = buildComboExportCopy(currentLanguage);
  const S        = 2;
  const W        = 820;
  const PAD      = 28;
  const HDR_H    = 72;
  ...
  ctx.fillText(exportCopy.teamStateLine, titleX, 50);
  ...
  for (let i = 0; i < n; i++) {
    ...
    ctx.fillText(exportCopy.scoreLabel, scoreX + 50, scoreY - 14);
