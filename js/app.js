...
function loadResearchModule() {
  if (!researchModulePromise) {
    researchModulePromise = import('./app-research.js?v=20260720_213712').catch((err) => {
...
function loadLoyaltyModule() {
  if (!loyaltyModulePromise) {
    loyaltyModulePromise = import('./loyalty-spa.js?v=20260720_213712').catch((error) => {
...
function loadExportModule() {
  if (!exportModulePromise) {
    exportModulePromise = import('./app-export.js?v=20260720_213712').catch((error) => {
...
function loadArcadeModule() {
  if (!arcadeModulePromise) {
    arcadeModulePromise = import('./arcade-spa.js?v=20260720_213712').catch((error) => {
  ...
  function onTabActivated(tabName) {
    ...
    if (tabName === 'edenMap' && !_edenMapReady && !_edenMapBooting) {
      ...
      loadTabTemplate('edenMap').then(() => {
        ...
        import('./eden-map.js?v=20260720_213712')
    ...
    if (tabName === 'heroes' && !_heroesTabReady) {
      ...
      import('./app-hero-atlas.js?v=20260720_213712')
        .then((mod) => {
          mod.renderHeroesTab();
    ...
    if (tabName === 'strife' && !_strifeReady) {
      ...
      _strifeBooting = true;
      import('./app-strife.js?v=20260720_213712')
    ...
    if (tabName === 'youtube' && !_youtubeReady && !_youtubeBooting) {
      ...
      import('./youtube-v14.js?v=20260720_213712')
        .then((mod) => {
