...
function loadResearchModule() {
  if (!researchModulePromise) {
    researchModulePromise = import('./app-research.js?v=20260925_133527').catch((err) => {
...
function loadExportModule() {
  if (!exportModulePromise) {
    exportModulePromise = import('./app-export.js?v=20260925_133527').catch((error) => {
...
function loadArcadeModule() {
  if (!arcadeModulePromise) {
    arcadeModulePromise = import('./arcade-spa.js?v=20260925_133527').catch((error) => {
  ...
  function onTabActivated(tabName) {
    ...
    if (tabName === 'edenMap' && !_edenMapReady && !_edenMapBooting) {
      ...
      loadTabTemplate('edenMap').then(() => {
        ...
        import('./eden-hub.js?v=20260925_133527')
          .then((hub) => hub.bootEdenHub())
          .then(() => import('./eden-map.js?v=20260925_133527'))
    ...
    if (tabName === 'heroes' && !_heroesTabReady) {
      ...
      import('./app-hero-atlas.js?v=20260925_133527')
        .then((mod) => {
          mod.renderHeroesTab();
    ...
    if (tabName === 'artifact' && !_artifactReady) {
      ...
      import('./app-artifact.js?v=20260925_133527')
        .then(async (mod) => {
    ...
    if (tabName === 'classDevelopment' && !_classDevelopmentReady) {
      if (_classDevelopmentBooting) return;
      _classDevelopmentBooting = true;
      import('./class-development.js')
        .then((mod) => {
          _classDevelopmentReady = mod.initClassDevelopment();
        })
        .catch((err) => {
          console.error('Class Development Hub failed to load', err);
          if (isDynamicImportLoadFailure(err)) {
            recoverFromStaleAssetGraph(err);
            return;
          }
          renderTabLoadError(document.getElementById('classDevelopmentRoot'), 'classDevelopment');
        })
        .finally(() => {
          _classDevelopmentBooting = false;
        });
    }
    if (tabName === 'strife' && !_strifeReady) {
      if (_strifeBooting) return;
      _strifeBooting = true;
      import('./app-strife.js?v=20260925_133527')
    ...
    if (tabName === 'youtube' && !_youtubeReady && !_youtubeBooting) {
      ...
      import('./youtube-v14.js?v=20260925_133527')
        .then((mod) => {
...
async function startApp() {
  try {
    ...
    safeInit('siegeCallout', () => {
      // The one-time Velo's Rampart callout loads after the hub is idle, as its
...
function initKeyboardAwareLayout() {
  ...
  function hasEditableFocus() {
    return document.activeElement?.matches?.(editableSelector) === true;
  }

  function sync() {
    const viewportKeyboardOpen = vv ? vv.height < window.innerHeight * 0.85 : false;
    const keyboardOpen = mobileQuery.matches && (focusedEditable || viewportKeyboardOpen);
  ...
  document.addEventListener('focusout', () => {
    ...
    focusRecoveryTimer = setTimeout(() => {
      focusedEditable = hasEditableFocus();
      sync();
    }, 180);
  });
