...
const PLAYBOOK_SRC = 'tabs/eden-playbook.html?v=20260924_153352';
...
function refreshMapViewport() {
  ...
  requestAnimationFrame(() => {
    ...
    import('./eden-map.js?v=20260924_153352')
...
async function loadLoyalty(root, panel) {
  ...
  try {
    ...
    const module = await import('./loyalty-spa.js?v=20260924_153352');
...
async function loadBounty(panel) {
  ...
  try {
    ...
    if (!response.ok) throw new Error(`HTTP ${response.status}`);
    panel.innerHTML = await response.text();
    const module = await import('./bounty-guide.js?v=20260924_153352');
