...
const LOYALTY_SRC = 'tabs/loyalty.html?v=20260904_092804';
const BOUNTY_SRC = 'tabs/bounty-guide.html?v=20260904_092804';
const PLAYBOOK_SRC = 'tabs/eden-playbook.html?v=20260904_092804';
...
function refreshMapViewport() {
  ...
  requestAnimationFrame(() => {
    ...
    import('./eden-map.js?v=20260904_092804')
...
async function loadLoyalty(root, panel) {
  ...
  try {
    ...
    const module = await import('./loyalty-spa.js?v=20260904_092804');
...
async function loadBounty(panel) {
  ...
  try {
    ...
    if (!response.ok) throw new Error(`HTTP ${response.status}`);
    panel.innerHTML = await response.text();
    const module = await import('./bounty-guide.js?v=20260904_092804');
