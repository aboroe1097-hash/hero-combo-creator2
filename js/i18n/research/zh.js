// Display-only Research localization. Canonical IDs, costs, and calculations remain in tech-db.js.
// This module is deliberately literal data: no runtime translation or source-data import.

export const researchContent = {
  trees: {
    d1956263: {
      name: '快速生产',
      unlockCondition: '无',
      primaryResource: '仅资源',
      pages: [],
    },
    b2691f74: {
      name: '城镇发展',
      unlockCondition: '无',
      primaryResource: '仅资源',
      pages: [],
    },
    '375d1626': {
      name: '基础战斗',
      unlockCondition: '无',
      primaryResource: '勇气勋章',
      pages: [],
    },
    '4a932eb0': {
      name: '饰品生产',
      unlockCondition: '无',
      primaryResource: '资源',
      pages: [],
    },
    '802e7893': {
      name: '军团 I 研究',
      unlockCondition: '无',
      primaryResource: '资源',
      pages: [],
    },
    '38d12254': {
      name: '军团 II 研究',
      unlockCondition: '无',
      primaryResource: '资源',
      pages: [],
    },
    '5fef2163': {
      name: '军团 III 研究',
      unlockCondition: '无',
      primaryResource: '资源',
      pages: [],
    },
    cc2c9ee1: {
      name: '兵种军团研究',
      unlockCondition: '无',
      primaryResource: '资源',
      pages: [],
    },
    '3929cbb9': {
      name: '医疗设施改良',
      unlockCondition: '无',
      primaryResource: '资源',
      pages: [],
    },
    f915a84b: {
      name: '基础军事',
      unlockCondition: '无',
      primaryResource: '资源',
      pages: [],
    },
    footmen_training: {
      name: '步兵训练',
      unlockCondition: '研究院24级、城堡25级、兵营25级，战区纪念与战区冲突100%',
      primaryResource: '勇气勋章',
      pages: [],
    },
    archers_training: {
      name: '弓兵训练',
      unlockCondition: '研究院24级、城堡25级、弓兵营25级，战区纪念与战区冲突100%',
      primaryResource: '勇气勋章',
      pages: [],
    },
    cavalry_training: {
      name: '骑兵训练',
      unlockCondition: '研究院24级、城堡25级、马厩25级，战区纪念与战区冲突100%',
      primaryResource: '勇气勋章',
      pages: [],
    },
    f0595b07: {
      name: '战区纪念',
      unlockCondition: '首次王座争夺战后解锁',
      primaryResource: '勇气勋章',
      pages: [],
    },
    zone_conflict: {
      name: '战区冲突',
      unlockCondition: '无',
      primaryResource: '勇气勋章',
      pages: [],
    },
    '1a6ec06e': {
      name: '战争大师',
      unlockCondition: '无',
      primaryResource: '勇气勋章',
      pages: ['拆除', '攻城战斗', '标记与占领'],
    },
    city_defence: {
      name: '城防大师',
      unlockCondition: '无',
      primaryResource: '勇气勋章',
      pages: ['城防工事', '增援', '箭塔与攻城'],
    },
    defence_legion: {
      name: '守城军团科技',
      unlockCondition: '完成城防科技中的“团结坚守”后解锁',
      primaryResource: '勇气勋章',
      pages: [],
    },
    e8704053: {
      name: '高级士兵训练',
      unlockCondition: 'RoC S1赛季开始时解锁',
      primaryResource: '勇气勋章',
      pages: ['强化部队', '训练', '高级'],
    },
    b3581c97: {
      name: '帝国禁卫',
      unlockCondition: 'RoC S2赛季开始时解锁',
      primaryResource: '勇气勋章',
      pages: [],
    },
    '2b149bcb': {
      name: '守军集结',
      unlockCondition: 'RoC S2赛季开始、城堡30级且研究院30级',
      primaryResource: '勇气勋章',
      pages: ['守军基础', '战术', '集结上限'],
    },
    '3f5ebb34': {
      name: '战争艺术·突袭',
      unlockCondition: 'Eden S3赛季开始时解锁',
      primaryResource: '双勋章',
      pages: ['武装部队', '攻城', '战争徽章'],
    },
    '3ff71437': {
      name: '战争艺术·防御',
      unlockCondition: 'Eden S3赛季开始时解锁',
      primaryResource: '双勋章',
      pages: ['坚守阵线', '攻城指令', '战争徽章'],
    },
    art_of_war_command: {
      name: '战争艺术·指挥',
      unlockCondition: 'Eden SX1赛季开始时解锁',
      primaryResource: '双勋章',
      pages: ['谋略与信念', '无畏打击', '指挥'],
    },
    '77db4a78': {
      name: '崇高战士',
      unlockCondition: 'Eden S4赛季开始时解锁',
      primaryResource: '战争徽章',
      pages: [],
    },
    '24ffc526': {
      name: '掠袭军团',
      unlockCondition: 'Eden S4赛季开始时解锁',
      primaryResource: '双勋章',
      pages: ['前排', '中排', '后排'],
    },
    '2c49bd2a': {
      name: '稳固战术',
      unlockCondition: 'Eden SX1赛季开始时解锁',
      primaryResource: '战争徽章',
      pages: ['攻城攻击', '攻城防御', '野战', '兵种战力', '快速训练', '高级'],
    },
    fc190e81: {
      name: '龙血祝福',
      unlockCondition: 'Eden SX2赛季开始且掠袭军团已满级',
      primaryResource: '战争徽章',
      pages: ['基础', '阵势', '龙之力'],
    },
    '19ae0569': {
      name: '崇高军团',
      unlockCondition: 'Eden X8赛季开始时解锁',
      primaryResource: '战争徽章',
      pages: ['崇高军团'],
    },
      '9c5d4006': {
      name: '近战军团 - 防御',
      unlockCondition: 'Eden X12赛季开始时解锁',
      primaryResource: '战争徽章',
      pages: ['近战军团 - 防御'],
    },
    'b57d6bf9': {
      name: '近战军团 - 冲锋',
      unlockCondition: 'Eden X12赛季开始时解锁',
      primaryResource: '战争徽章',
      pages: ['近战军团 - 冲锋'],
    },
},
  nodes: {
    'd1956263:node_1': {
      name: '快速增长',
      buff: '解锁农田快速生产',
      effects: [],
    },
    'd1956263:node_2': {
      name: '烈酒',
      buff: '解锁酿酒厂快速生产',
      effects: [],
    },
    'd1956263:node_3': {
      name: '伐林',
      buff: '解锁木材场快速生产',
      effects: [],
    },
    'd1956263:node_4': {
      name: '快速燃烧',
      buff: '解锁木炭工坊快速生产',
      effects: [],
    },
    'd1956263:node_5': {
      name: '破土开采',
      buff: '解锁采石场快速生产',
      effects: [],
    },
    'd1956263:node_6': {
      name: '快速冶炼',
      buff: '解锁铁矿场快速生产',
      effects: [],
    },
    'd1956263:node_7': {
      name: '工人动员',
      buff: '建筑升级时恢复快速生产成功率',
      effects: [],
    },
    'd1956263:node_8': {
      name: '狂热增长',
      buff: '农田快速生产产量每级 +5%',
      effects: [],
    },
    'd1956263:node_9': {
      name: '活性酵母',
      buff: '酿酒厂快速生产产量每级 +5%',
      effects: [],
    },
    'd1956263:node_10': {
      name: '整树采伐',
      buff: '木材场快速生产产量每级 +5%',
      effects: [],
    },
    'd1956263:node_11': {
      name: '干燥仓储',
      buff: '木炭工坊快速生产产量每级 +5%',
      effects: [],
    },
    'd1956263:node_12': {
      name: '整材·石材利用',
      buff: '采石场快速生产产量每级 +5%',
      effects: [],
    },
    'd1956263:node_13': {
      name: '冶金',
      buff: '铁矿场快速生产产量每级 +5%',
      effects: [],
    },
    'd1956263:node_14': {
      name: '意志与行动',
      buff: '每日首次快速生产必定成功',
      effects: [],
    },
    'd1956263:node_15': {
      name: '稳定改良',
      buff: '快速生产成功率每级 +2%',
      effects: [],
    },
    'b2691f74:node_1': {
      name: '肥料',
      buff: '食物产量每级+3%',
      effects: [],
    },
    'b2691f74:node_2': {
      name: '高级糖',
      buff: '麦酒产量每级+3%',
      effects: [],
    },
    'b2691f74:node_3': {
      name: '全面砍伐 I',
      buff: '木材产量每级+3%',
      effects: [],
    },
    'b2691f74:node_4': {
      name: '木炭加工',
      buff: '木炭产量每级+3%',
      effects: [],
    },
    'b2691f74:node_5': {
      name: '石工',
      buff: '大理石产量每级+3%',
      effects: [],
    },
    'b2691f74:node_6': {
      name: '高温熔炉I',
      buff: '铁矿产量每级+3%',
      effects: [],
    },
    'b2691f74:node_7': {
      name: '点金之触',
      buff: '金币产量每级+3%',
      effects: [],
    },
    'b2691f74:node_8': {
      name: '快速运输I',
      buff: '食物采集速度每级+3%',
      effects: [],
    },
    'b2691f74:node_9': {
      name: '锋利木材斧I',
      buff: '木材采集速度每级+3%',
      effects: [],
    },
    'b2691f74:node_10': {
      name: '高效采矿',
      buff: '大理石采集速度每级+3%',
      effects: [],
    },
    'b2691f74:node_11': {
      name: '合金精炼I',
      buff: '铁矿采集速度每级+3%',
      effects: [],
    },
    'b2691f74:node_12': {
      name: '宝石精炼I',
      buff: '宝石采集速度每级+3%',
      effects: [],
    },
    'b2691f74:node_13': {
      name: '仓库扩容 I',
      buff: '资源存储上限每级 +90k',
      effects: [],
    },
    'b2691f74:node_14': {
      name: '土木工程I',
      buff: '建筑速度每级+1%',
      effects: [],
    },
    '375d1626:node_1': {
      name: '勇气嘉奖',
      buff: '击败掠夺者时有概率获得勇气勋章',
      effects: [],
    },
    '375d1626:node_2': {
      name: '步兵晋升I',
      buff: '1-3阶步兵可晋升',
      effects: [],
    },
    '375d1626:node_3': {
      name: '弓兵晋升I',
      buff: '1-3阶弓兵可晋升',
      effects: [],
    },
    '375d1626:node_4': {
      name: '骑兵晋升I',
      buff: '1-3阶骑兵可晋升',
      effects: [],
    },
    '375d1626:node_5': {
      name: '为民而战',
      buff: '击败掠夺者获得的资源',
      effects: [],
    },
    '375d1626:node_6': {
      name: '初级叛军猎杀者',
      buff: '对 1-5 级掠夺者的攻击',
      effects: [],
    },
    '375d1626:node_7': {
      name: '初级帝国守卫',
      buff: '对 1-5 级掠夺者的防御',
      effects: [],
    },
    '375d1626:node_8': {
      name: '步兵晋升II',
      buff: '4-6阶步兵可晋升',
      effects: [],
    },
    '375d1626:node_9': {
      name: '弓兵晋升II',
      buff: '4-6阶弓兵可晋升',
      effects: [],
    },
    '375d1626:node_10': {
      name: '骑兵晋升II',
      buff: '4-6阶骑兵可晋升',
      effects: [],
    },
    '375d1626:node_11': {
      name: '英雄训练',
      buff: '击败掠夺者获得的英雄经验',
      effects: [],
    },
    '375d1626:node_12': {
      name: '中级叛军猎杀者',
      buff: '对 6-10 级掠夺者的攻击',
      effects: [],
    },
    '375d1626:node_13': {
      name: '中级帝国守卫',
      buff: '对 6-10 级掠夺者的防御',
      effects: [],
    },
    '375d1626:node_14': {
      name: '战地急救',
      buff: '与掠夺者战斗时降低伤兵比例',
      effects: [],
    },
    '375d1626:node_15': {
      name: '高级叛军猎杀者',
      buff: '对 11-15 级掠夺者的攻击',
      effects: [],
    },
    '375d1626:node_16': {
      name: '高级帝国守卫',
      buff: '对 11-15 级掠夺者的防御',
      effects: [],
    },
    '4a932eb0:node_1': {
      name: '熔炉扩编',
      buff: '解锁第一个材料制作槽位',
      effects: [],
    },
    '4a932eb0:node_2': {
      name: '基础精炼',
      buff: '提升生产速度',
      effects: [],
    },
    '4a932eb0:node_3': {
      name: '基础加工',
      buff: '提升材料制作速度',
      effects: [],
    },
    '4a932eb0:node_4': {
      name: '熔炉扩编',
      buff: '解锁第二个材料制作槽位',
      effects: [],
    },
    '4a932eb0:node_5': {
      name: '基础精炼',
      buff: '提升生产速度',
      effects: [],
    },
    '4a932eb0:node_6': {
      name: '基础加工',
      buff: '提升材料制作速度',
      effects: [],
    },
    '4a932eb0:node_7': {
      name: '熔炉扩编',
      buff: '解锁第三个材料制作槽位',
      effects: [],
    },
    '4a932eb0:node_8': {
      name: '武器加工',
      buff: '提升武器制作速度',
      effects: [],
    },
    '4a932eb0:node_9': {
      name: '头盔制作',
      buff: '提升头盔制作速度',
      effects: [],
    },
    '4a932eb0:node_10': {
      name: '护甲匠',
      buff: '提升护甲制作速度',
      effects: [],
    },
    '4a932eb0:node_11': {
      name: '战靴工匠',
      buff: '提升战靴制作速度',
      effects: [],
    },
    '4a932eb0:node_12': {
      name: '珠宝匠',
      buff: '提升护符制作速度',
      effects: [],
    },
    '4a932eb0:node_13': {
      name: '饰品加工',
      buff: '提升饰品制作速度',
      effects: [],
    },
    '4a932eb0:node_14': {
      name: '工坊扩编',
      buff: '额外解锁一个龙晶工坊',
      effects: [],
    },
    '4a932eb0:node_15': {
      name: '基础精炼',
      buff: '提升生产速度',
      effects: [],
    },
    '4a932eb0:node_16': {
      name: '基础加工',
      buff: '提升材料制作速度',
      effects: [],
    },
    '4a932eb0:node_17': {
      name: '熔炉扩编',
      buff: '解锁第四个材料制作槽位',
      effects: [],
    },
    '4a932eb0:node_18': {
      name: '武器熔炉',
      buff: '锻造武器时降低龙晶消耗',
      effects: [],
    },
    '4a932eb0:node_19': {
      name: '头盔熔炉',
      buff: '锻造头盔时降低龙晶消耗',
      effects: [],
    },
    '4a932eb0:node_20': {
      name: '护甲熔炉',
      buff: '锻造护甲时降低龙晶消耗',
      effects: [],
    },
    '4a932eb0:node_21': {
      name: '战靴熔炉',
      buff: '锻造战靴时降低龙晶消耗',
      effects: [],
    },
    '4a932eb0:node_22': {
      name: '护符熔炉',
      buff: '锻造护符时降低龙晶消耗',
      effects: [],
    },
    '4a932eb0:node_23': {
      name: '饰品熔炉',
      buff: '锻造饰品时降低龙晶消耗',
      effects: [],
    },
    '4a932eb0:node_24': {
      name: '熔炉扩编',
      buff: '解锁第五个材料制作槽位',
      effects: [],
    },
    '4a932eb0:node_25': {
      name: '新型材料',
      buff: '解锁绿色材料制作',
      effects: [],
    },
    '4a932eb0:node_26': {
      name: '材料回收',
      buff: '可将材料分解为龙晶',
      effects: [],
    },
    '4a932eb0:node_27': {
      name: '批量回收',
      buff: '提升分解获得的龙晶数量',
      effects: [],
    },
    '4a932eb0:node_28': {
      name: '精通回收',
      buff: '提升装备分解时的返还量',
      effects: [],
    },
    '802e7893:node_1': {
      name: '英雄任命',
      buff: '解锁军团 I 前排',
      effects: [],
    },
    '802e7893:node_2': {
      name: '高速采集',
      buff: '采集速度军团I +25%',
      effects: ['军团 I 采集速度'],
    },
    '802e7893:node_3': {
      name: '强化体魄',
      buff: '部队负重军团I +25%',
      effects: ['军团 I 部队负重'],
    },
    '802e7893:node_4': {
      name: '步兵抗性',
      buff: '步兵军团I防御 +35%',
      effects: ['军团 I 步兵防御'],
    },
    '802e7893:node_5': {
      name: '弓兵抗性',
      buff: '弓兵军团I防御 +35%',
      effects: ['军团 I 弓兵防御'],
    },
    '802e7893:node_6': {
      name: '骑兵抗性',
      buff: '骑兵军团I防御 +35%',
      effects: ['军团 I 骑兵防御'],
    },
    '802e7893:node_7': {
      name: '统帅旗帜',
      buff: '解锁军团 I 加速',
      effects: [],
    },
    '802e7893:node_8': {
      name: '英雄协作',
      buff: '解锁军团 I 中排',
      effects: [],
    },
    '802e7893:node_9': {
      name: '召回号令',
      buff: '解锁军团 I 召回',
      effects: [],
    },
    '802e7893:node_10': {
      name: '合金马蹄铁',
      buff: '行军速度军团I +25%',
      effects: ['军团 I 行军速度'],
    },
    '802e7893:node_11': {
      name: '补给单位',
      buff: '耐久恢复速度军团I +35%',
      effects: ['军团 I 耐久恢复速度'],
    },
    '802e7893:node_12': {
      name: '步兵突击',
      buff: '步兵军团I攻击 +35%',
      effects: ['军团 I 步兵攻击'],
    },
    '802e7893:node_13': {
      name: '弓兵突击',
      buff: '弓兵军团I攻击 +35%',
      effects: ['军团 I 弓兵攻击'],
    },
    '802e7893:node_14': {
      name: '骑兵突击',
      buff: '骑兵军团I攻击 +35%',
      effects: ['军团 I 骑兵攻击'],
    },
    '802e7893:node_15': {
      name: '英雄指挥',
      buff: '解锁军团 I 后排',
      effects: [],
    },
    '38d12254:node_1': {
      name: '英雄任命',
      buff: '解锁军团 II 前排',
      effects: [],
    },
    '38d12254:node_2': {
      name: '高速采集',
      buff: '采集速度军团II +25%',
      effects: ['军团 II 采集速度'],
    },
    '38d12254:node_3': {
      name: '强化体魄',
      buff: '部队负重军团II +25%',
      effects: ['军团 II 部队负重'],
    },
    '38d12254:node_4': {
      name: '步兵抗性',
      buff: '步兵军团II防御 +35%',
      effects: ['军团 II 步兵防御'],
    },
    '38d12254:node_5': {
      name: '弓兵抗性',
      buff: '弓兵军团II防御 +35%',
      effects: ['军团 II 弓兵防御'],
    },
    '38d12254:node_6': {
      name: '骑兵抗性',
      buff: '骑兵军团II防御 +35%',
      effects: ['军团 II 骑兵防御'],
    },
    '38d12254:node_7': {
      name: '统帅旗帜',
      buff: '解锁军团 II 加速',
      effects: [],
    },
    '38d12254:node_8': {
      name: '英雄协作',
      buff: '解锁军团 II 中排',
      effects: [],
    },
    '38d12254:node_9': {
      name: '召回号令',
      buff: '解锁军团 II 召回',
      effects: [],
    },
    '38d12254:node_10': {
      name: '合金马蹄铁',
      buff: '行军速度军团II +25%',
      effects: ['军团 II 行军速度'],
    },
    '38d12254:node_11': {
      name: '补给单位',
      buff: '耐久恢复速度军团II +35%',
      effects: ['军团 II 耐久恢复速度'],
    },
    '38d12254:node_12': {
      name: '步兵突击',
      buff: '步兵军团II攻击 +35%',
      effects: ['军团 II 步兵攻击'],
    },
    '38d12254:node_13': {
      name: '弓兵突击',
      buff: '弓兵军团II攻击 +35%',
      effects: ['军团 II 弓兵攻击'],
    },
    '38d12254:node_14': {
      name: '骑兵突击',
      buff: '骑兵军团II攻击 +35%',
      effects: ['军团 II 骑兵攻击'],
    },
    '38d12254:node_15': {
      name: '英雄指挥',
      buff: '解锁军团 II 后排',
      effects: [],
    },
    '5fef2163:node_1': {
      name: '英雄任命',
      buff: '解锁军团 III 前排',
      effects: [],
    },
    '5fef2163:node_2': {
      name: '高速采集',
      buff: '采集速度军团III +25%',
      effects: ['军团 III 采集速度'],
    },
    '5fef2163:node_3': {
      name: '强化体魄',
      buff: '部队负重军团III +25%',
      effects: ['军团 III 部队负重'],
    },
    '5fef2163:node_4': {
      name: '步兵抗性',
      buff: '步兵军团III防御 +35%',
      effects: ['军团 III 步兵防御'],
    },
    '5fef2163:node_5': {
      name: '弓兵抗性',
      buff: '弓兵军团III防御 +35%',
      effects: ['军团 III 弓兵防御'],
    },
    '5fef2163:node_6': {
      name: '骑兵抗性',
      buff: '骑兵军团III防御 +35%',
      effects: ['军团 III 骑兵防御'],
    },
    '5fef2163:node_7': {
      name: '统帅旗帜',
      buff: '解锁军团 III 加速',
      effects: [],
    },
    '5fef2163:node_8': {
      name: '英雄协作',
      buff: '解锁军团 III 中排',
      effects: [],
    },
    '5fef2163:node_9': {
      name: '召回号令',
      buff: '解锁军团 III 召回',
      effects: [],
    },
    '5fef2163:node_10': {
      name: '合金马蹄铁',
      buff: '行军速度军团III +25%',
      effects: ['军团 III 行军速度'],
    },
    '5fef2163:node_11': {
      name: '补给单位',
      buff: '耐久恢复速度军团III +35%',
      effects: ['军团 III 耐久恢复速度'],
    },
    '5fef2163:node_12': {
      name: '步兵突击',
      buff: '步兵军团III攻击 +35%',
      effects: ['军团 III 步兵攻击'],
    },
    '5fef2163:node_13': {
      name: '弓兵突击',
      buff: '弓兵军团III攻击 +35%',
      effects: ['军团 III 弓兵攻击'],
    },
    '5fef2163:node_14': {
      name: '骑兵突击',
      buff: '骑兵军团III攻击 +35%',
      effects: ['军团 III 骑兵攻击'],
    },
    '5fef2163:node_15': {
      name: '英雄指挥',
      buff: '解锁军团 III 后排',
      effects: [],
    },
    'cc2c9ee1:node_1': {
      name: '英雄任命',
      buff: '部署 1 名英雄',
      effects: [],
    },
    'cc2c9ee1:node_2': {
      name: '高速采集',
      buff: '采集速度',
      effects: [],
    },
    'cc2c9ee1:node_3': {
      name: '强化体魄',
      buff: '部队负重',
      effects: [],
    },
    'cc2c9ee1:node_4': {
      name: '步兵抗性',
      buff: '防御',
      effects: [],
    },
    'cc2c9ee1:node_5': {
      name: '骑兵抗性',
      buff: '防御',
      effects: [],
    },
    'cc2c9ee1:node_6': {
      name: '弓兵抗性',
      buff: '防御',
      effects: [],
    },
    'cc2c9ee1:node_7': {
      name: '统帅旗帜',
      buff: '加速',
      effects: [],
    },
    'cc2c9ee1:node_8': {
      name: '英雄协作',
      buff: '部署 2 名英雄',
      effects: [],
    },
    'cc2c9ee1:node_9': {
      name: '召回号令',
      buff: '部队召回',
      effects: [],
    },
    'cc2c9ee1:node_10': {
      name: '合金马蹄铁',
      buff: '行军速度',
      effects: [],
    },
    'cc2c9ee1:node_11': {
      name: '补给单位',
      buff: '体力恢复速度',
      effects: [],
    },
    'cc2c9ee1:node_12': {
      name: '步兵突击',
      buff: '攻击',
      effects: [],
    },
    'cc2c9ee1:node_13': {
      name: '弓兵突击',
      buff: '攻击',
      effects: [],
    },
    'cc2c9ee1:node_14': {
      name: '骑兵突击',
      buff: '攻击',
      effects: [],
    },
    'cc2c9ee1:node_15': {
      name: '英雄指挥',
      buff: '部署 3 名英雄',
      effects: [],
    },
    '3929cbb9:node_1': {
      name: '步兵治疗',
      buff: '步兵治疗消耗',
      effects: [],
    },
    '3929cbb9:node_2': {
      name: '弓兵治疗',
      buff: '弓兵治疗消耗',
      effects: [],
    },
    '3929cbb9:node_3': {
      name: '骑兵治疗',
      buff: '骑兵治疗消耗',
      effects: [],
    },
    '3929cbb9:node_4': {
      name: '临时病床',
      buff: '城堡伤兵容量',
      effects: [],
    },
    '3929cbb9:node_5': {
      name: '步兵急救',
      buff: '步兵治疗速度',
      effects: [],
    },
    '3929cbb9:node_6': {
      name: '弓兵急救',
      buff: '弓兵治疗速度',
      effects: [],
    },
    '3929cbb9:node_7': {
      name: '兽医',
      buff: '骑兵治疗速度',
      effects: [],
    },
    '3929cbb9:node_8': {
      name: '教堂医院',
      buff: '额外增加一个医疗帐篷',
      effects: [],
    },
    '3929cbb9:node_9': {
      name: '医疗设施I',
      buff: '城堡伤兵容量',
      effects: [],
    },
    '3929cbb9:node_10': {
      name: '庇护',
      buff: '伤兵超额容量',
      effects: [],
    },
    '3929cbb9:node_11': {
      name: '强效治疗',
      buff: '单人攻城时降低死亡率',
      effects: [],
    },
    'f915a84b:node_1': {
      name: '步兵征募',
      buff: '步兵训练速度',
      effects: [],
    },
    'f915a84b:node_2': {
      name: '弓兵征募',
      buff: '弓兵训练速度',
      effects: [],
    },
    'f915a84b:node_3': {
      name: '骑兵征募',
      buff: '骑兵训练速度',
      effects: [],
    },
    'f915a84b:node_4': {
      name: '征召',
      buff: '额外增加一个营地',
      effects: [],
    },
    'f915a84b:node_5': {
      name: '步兵征募',
      buff: '步兵训练速度',
      effects: [],
    },
    'f915a84b:node_6': {
      name: '弓兵征募',
      buff: '弓兵训练速度',
      effects: [],
    },
    'f915a84b:node_7': {
      name: '骑兵征募',
      buff: '骑兵训练速度',
      effects: [],
    },
    'f915a84b:node_8': {
      name: '全金属护甲',
      buff: 'HP',
      effects: [],
    },
    'f915a84b:node_9': {
      name: '兵营',
      buff: '步兵训练数量',
      effects: [],
    },
    'f915a84b:node_10': {
      name: '弓兵营地',
      buff: '弓兵训练数量',
      effects: [],
    },
    'f915a84b:node_11': {
      name: '骑兵训练营',
      buff: '骑兵训练数量',
      effects: [],
    },
    'f915a84b:node_12': {
      name: '致命武器',
      buff: '单人攻城胜利时的敌军死亡率',
      effects: [],
    },
    'footmen_training:node_1': {
      name: '强力打击',
      buff: '攻击 10%',
      effects: [],
    },
    'footmen_training:node_2': {
      name: '大师护甲',
      buff: '防御 10%',
      effects: [],
    },
    'footmen_training:node_3': {
      name: '大师护盾',
      buff: '步兵对骑兵克制 10%',
      effects: [],
    },
    'footmen_training:node_4': {
      name: '加固护盾',
      buff: '步兵负重 100%',
      effects: [],
    },
    'footmen_training:node_5': {
      name: '坚守阵地',
      buff: '步兵行军速度 100%',
      effects: [],
    },
    'footmen_training:node_6': {
      name: '盾牌大师',
      buff: '步兵对弓兵克制 10%',
      effects: [],
    },
    'footmen_training:node_7': {
      name: '强力打击',
      buff: '攻击 20%',
      effects: [],
    },
    'footmen_training:node_8': {
      name: '大师护甲',
      buff: '防御 20%',
      effects: [],
    },
    'footmen_training:node_9': {
      name: '步兵强化',
      buff: '强化步兵',
      effects: [],
    },
    'footmen_training:node_10': {
      name: '步兵征募',
      buff: '训练消耗 -15%',
      effects: [],
    },
    'footmen_training:node_11': {
      name: '步兵征召',
      buff: '额外晋升兵力 +16500',
      effects: [],
    },
    'footmen_training:node_12': {
      name: '皇家护盾',
      buff: '步兵对弓兵克制 10%',
      effects: [],
    },
    'footmen_training:node_13': {
      name: '步兵晋升III',
      buff: '从 T7 晋升至 T9',
      effects: [],
    },
    'footmen_training:node_14': {
      name: '巨剑',
      buff: '守城攻击 30%',
      effects: [],
    },
    'footmen_training:node_15': {
      name: '锁链护甲',
      buff: '防御 30%',
      effects: [],
    },
    'footmen_training:node_16': {
      name: '帝国守卫(步兵)',
      buff: '解锁T9',
      effects: [],
    },
    'archers_training:node_1': {
      name: '强化箭矢',
      buff: '攻击 10%',
      effects: [],
    },
    'archers_training:node_2': {
      name: '强化护臂',
      buff: '防御 10%',
      effects: [],
    },
    'archers_training:node_3': {
      name: '目标弱点',
      buff: '步兵对骑兵克制 10%',
      effects: [],
    },
    'archers_training:node_4': {
      name: '专注火力',
      buff: '步兵负重 100%',
      effects: [],
    },
    'archers_training:node_5': {
      name: '龙皮护甲',
      buff: '步兵行军速度 100%',
      effects: [],
    },
    'archers_training:node_6': {
      name: '致命瞄准',
      buff: '步兵对弓兵克制 10%',
      effects: [],
    },
    'archers_training:node_7': {
      name: '强化箭矢',
      buff: '攻击 20%',
      effects: [],
    },
    'archers_training:node_8': {
      name: '乌木护臂',
      buff: '防御 20%',
      effects: [],
    },
    'archers_training:node_9': {
      name: '弓兵强化',
      buff: '强化弓兵',
      effects: [],
    },
    'archers_training:node_10': {
      name: '弓兵征募',
      buff: '训练消耗 -15%',
      effects: [],
    },
    'archers_training:node_11': {
      name: '弓兵征召',
      buff: '额外晋升兵力 +16500',
      effects: [],
    },
    'archers_training:node_12': {
      name: '狩猎',
      buff: '步兵对弓兵克制 10%',
      effects: [],
    },
    'archers_training:node_13': {
      name: '弓兵晋升III',
      buff: '从 T7 晋升至 T9',
      effects: [],
    },
    'archers_training:node_14': {
      name: '箭雨',
      buff: '攻城攻击 30%',
      effects: [],
    },
    'archers_training:node_15': {
      name: '剧毒箭头',
      buff: '攻击 30%',
      effects: [],
    },
    'archers_training:node_16': {
      name: '解锁 9 阶弓兵',
      buff: '解锁T9',
      effects: [],
    },
    'cavalry_training:node_1': {
      name: '炽烈冲锋',
      buff: '攻击 10%',
      effects: [],
    },
    'cavalry_training:node_2': {
      name: '强化马铠',
      buff: '防御 10%',
      effects: [],
    },
    'cavalry_training:node_3': {
      name: '推进万箭穿心',
      buff: '步兵对骑兵克制 10%',
      effects: [],
    },
    'cavalry_training:node_4': {
      name: '运输货车',
      buff: '步兵负重 100%',
      effects: [],
    },
    'cavalry_training:node_5': {
      name: '纯血战马',
      buff: '步兵行军速度 100%',
      effects: [],
    },
    'cavalry_training:node_6': {
      name: '猛攻',
      buff: '步兵对弓兵克制 10%',
      effects: [],
    },
    'cavalry_training:node_7': {
      name: '凶猛冲锋',
      buff: '攻击 20%',
      effects: [],
    },
    'cavalry_training:node_8': {
      name: '强化马铠',
      buff: '防御 20%',
      effects: [],
    },
    'cavalry_training:node_9': {
      name: '骑兵强化',
      buff: '强化骑兵',
      effects: [],
    },
    'cavalry_training:node_10': {
      name: '骑兵征募',
      buff: '训练消耗 -15%',
      effects: [],
    },
    'cavalry_training:node_11': {
      name: '侍从训练',
      buff: '额外晋升兵力 +16500',
      effects: [],
    },
    'cavalry_training:node_12': {
      name: '征途',
      buff: '步兵对弓兵克制 10%',
      effects: [],
    },
    'cavalry_training:node_13': {
      name: '骑兵晋升III',
      buff: '从 T7 晋升至 T9',
      effects: [],
    },
    'cavalry_training:node_14': {
      name: '平原遭遇战',
      buff: '攻击 30%',
      effects: [],
    },
    'cavalry_training:node_15': {
      name: '乌木马铠',
      buff: '防御 30%',
      effects: [],
    },
    'cavalry_training:node_16': {
      name: '帝国守卫(骑兵)',
      buff: '解锁T9',
      effects: [],
    },
    'f0595b07:node_1': {
      name: '全面生产',
      buff: '采集速度',
      effects: [],
    },
    'f0595b07:node_2': {
      name: '远征军',
      buff: '资源采集量',
      effects: [],
    },
    'f0595b07:node_3': {
      name: '高级奖励',
      buff: '解锁 4-6 级奖励',
      effects: [],
    },
    'f0595b07:node_4': {
      name: '武器专家',
      buff: '获得全部积分',
      effects: [],
    },
    'f0595b07:node_5': {
      name: '快速清剿',
      buff: '攻击掠夺者时的行军速度',
      effects: [],
    },
    'f0595b07:node_6': {
      name: '战争备战训练',
      buff: '训练速度',
      effects: [],
    },
    'f0595b07:node_7': {
      name: '负重提升',
      buff: '部队负重',
      effects: [],
    },
    'f0595b07:node_8': {
      name: '战争备战容量',
      buff: '训练容量',
      effects: [],
    },
    'f0595b07:node_9': {
      name: '超级奖励',
      buff: '解锁 7-9 级奖励',
      effects: [],
    },
    'f0595b07:node_10': {
      name: '激励·采集',
      buff: '采集积分',
      effects: [],
    },
    'f0595b07:node_11': {
      name: '激励·击杀掠夺者',
      buff: '攻击掠夺者积分',
      effects: [],
    },
    'f0595b07:node_12': {
      name: '激励·建筑',
      buff: '建造积分',
      effects: [],
    },
    'f0595b07:node_13': {
      name: '鼓舞·智慧',
      buff: '战争徽章积分',
      effects: [],
    },
    'f0595b07:node_14': {
      name: '激励·科技研究',
      buff: '科技研究积分',
      effects: [],
    },
    'f0595b07:node_15': {
      name: '鼓舞·成长',
      buff: '英雄升级积分',
      effects: [],
    },
    'f0595b07:node_16': {
      name: '激励·击杀敌军',
      buff: '击杀敌军积分',
      effects: [],
    },
    'f0595b07:node_17': {
      name: '鼓舞·征募',
      buff: '征募积分',
      effects: [],
    },
    'f0595b07:node_18': {
      name: '武器专家',
      buff: '获得全部积分',
      effects: [],
    },
    'f0595b07:node_19': {
      name: '再来一份奖励！',
      buff: '双倍奖励',
      effects: [],
    },
    'zone_conflict:node_1': {
      name: '医疗保障',
      buff: '医疗保障',
      effects: [],
    },
    'zone_conflict:node_2': {
      name: '背水一战',
      buff: '背水一战',
      effects: [],
    },
    'zone_conflict:node_3': {
      name: '战斗渴望',
      buff: '战斗渴望',
      effects: [],
    },
    'zone_conflict:node_4': {
      name: '修筑城墙',
      buff: '修筑城墙',
      effects: [],
    },
    'zone_conflict:node_5': {
      name: '临时城防工事',
      buff: '临时防御工事',
      effects: [],
    },
    'zone_conflict:node_6': {
      name: '和平守护者',
      buff: '和平守卫',
      effects: [],
    },
    'zone_conflict:node_7': {
      name: '战役韧性',
      buff: '战役韧性',
      effects: [],
    },
    'zone_conflict:node_8': {
      name: '医疗保障',
      buff: '保障 II',
      effects: [],
    },
    'zone_conflict:node_9': {
      name: '联合阵线',
      buff: '联合效果：医疗',
      effects: [],
    },
    'zone_conflict:node_10': {
      name: '联合攻击',
      buff: '联合攻击',
      effects: [],
    },
    'zone_conflict:node_11': {
      name: '防御动员',
      buff: '防御动员',
      effects: [],
    },
    'zone_conflict:node_12': {
      name: '盟军攻击',
      buff: '盟军攻击',
      effects: [],
    },
    'zone_conflict:node_13': {
      name: '超级军团',
      buff: '解锁超级军团',
      effects: [],
    },
    'zone_conflict:node_14': {
      name: '碳纤维骨骼',
      buff: '碳纤维骨骼',
      effects: [],
    },
    'zone_conflict:node_15': {
      name: '剧毒孢子',
      buff: '剧毒孢子',
      effects: [],
    },
    '1a6ec06e:node_1': {
      name: '载荷改良',
      buff: '部队负重 +20%',
      effects: [],
    },
    '1a6ec06e:node_2': {
      name: '步兵突击',
      buff: '步兵拆除值 +10%',
      effects: [],
    },
    '1a6ec06e:node_3': {
      name: '弓兵突击',
      buff: '弓兵拆除值 +10%',
      effects: [],
    },
    '1a6ec06e:node_4': {
      name: '骑兵突击',
      buff: '骑兵拆除值 +10%',
      effects: [],
    },
    '1a6ec06e:node_5': {
      name: '全面突击',
      buff: '行军速度 +20%',
      effects: [],
    },
    '1a6ec06e:node_6': {
      name: '前排攻击',
      buff: '步兵攻城攻击 +20%',
      effects: [],
    },
    '1a6ec06e:node_7': {
      name: '弓兵征募',
      buff: '弓兵攻城攻击 +20%',
      effects: [],
    },
    '1a6ec06e:node_8': {
      name: '骑兵征募',
      buff: '骑兵攻城攻击 +20%',
      effects: [],
    },
    '1a6ec06e:node_9': {
      name: '重装护盾',
      buff: '步兵攻城防御 +20%',
      effects: [],
    },
    '1a6ec06e:node_10': {
      name: '铁甲',
      buff: '弓兵攻城防御 +20%',
      effects: [],
    },
    '1a6ec06e:node_11': {
      name: '重装骑手',
      buff: '骑兵攻城防御 +20%',
      effects: [],
    },
    '1a6ec06e:node_12': {
      name: '致命武器',
      buff: '单人玩家攻城胜利敌军死亡率 +20%',
      effects: [],
    },
    '1a6ec06e:node_13': {
      name: '敏捷',
      buff: '步兵战斗速度 +50',
      effects: [],
    },
    '1a6ec06e:node_14': {
      name: '快速射击',
      buff: '弓兵战斗速度 +50',
      effects: [],
    },
    '1a6ec06e:node_15': {
      name: '快马加鞭',
      buff: '骑兵战斗速度 +50',
      effects: [],
    },
    '1a6ec06e:node_16': {
      name: '解锁烟雾信号',
      buff: '攻城失败后，守方损失兵力且攻击与防御降低',
      effects: [],
    },
    '1a6ec06e:node_17': {
      name: '缴械',
      buff: '拆除后的攻击减益效果 +10%',
      effects: [],
    },
    '1a6ec06e:node_18': {
      name: '护甲穿透',
      buff: '拆除后的防御减益效果 +10%',
      effects: [],
    },
    '1a6ec06e:node_19': {
      name: '麻痹',
      buff: '拆除效果持续时间 +600',
      effects: [],
    },
    '1a6ec06e:node_20': {
      name: '步兵打击',
      buff: '步兵攻击 +20%',
      effects: [],
    },
    '1a6ec06e:node_21': {
      name: '弓兵打击',
      buff: '弓兵攻击 +20%',
      effects: [],
    },
    '1a6ec06e:node_22': {
      name: '骑兵打击',
      buff: '骑兵攻击 +20%',
      effects: [],
    },
    '1a6ec06e:node_23': {
      name: '步兵防御',
      buff: '步兵防御 +20%',
      effects: [],
    },
    '1a6ec06e:node_24': {
      name: '弓兵防御',
      buff: '弓兵防御 +20%',
      effects: [],
    },
    '1a6ec06e:node_25': {
      name: '骑兵防御',
      buff: '骑兵防御 +20%',
      effects: [],
    },
    '1a6ec06e:node_26': {
      name: '战斗伤痕',
      buff: '标记敌方城堡',
      effects: [],
    },
    '1a6ec06e:node_27': {
      name: '标记',
      buff: '对标记目标的攻击加成 +10%',
      effects: [],
    },
    '1a6ec06e:node_28': {
      name: '标记防御',
      buff: '对标记目标的防御加成 +10%',
      effects: [],
    },
    '1a6ec06e:node_29': {
      name: '持久标记',
      buff: '标记持续时间增加 +1200',
      effects: [],
    },
    '1a6ec06e:node_30': {
      name: '多重标记',
      buff: '标记上限增加 +2',
      effects: [],
    },
    '1a6ec06e:node_31': {
      name: '步兵突击',
      buff: '步兵拆除值 +10%',
      effects: [],
    },
    '1a6ec06e:node_32': {
      name: '弓兵突击',
      buff: '弓兵拆除值 +10%',
      effects: [],
    },
    '1a6ec06e:node_33': {
      name: '骑兵突击',
      buff: '骑兵拆除值 +10%',
      effects: [],
    },
    '1a6ec06e:node_34': {
      name: '杀戮',
      buff: '敌军战损 +20%',
      effects: [],
    },
    '1a6ec06e:node_35': {
      name: '前排攻击',
      buff: '步兵攻城攻击 +20%',
      effects: [],
    },
    '1a6ec06e:node_36': {
      name: '弓兵征募',
      buff: '弓兵攻城攻击 +20%',
      effects: [],
    },
    '1a6ec06e:node_37': {
      name: '骑兵征募',
      buff: '骑兵攻城攻击 +20%',
      effects: [],
    },
    '1a6ec06e:node_38': {
      name: '重装护盾',
      buff: '步兵攻城防御 +20%',
      effects: [],
    },
    '1a6ec06e:node_39': {
      name: '铁甲',
      buff: '弓兵攻城防御 +20%',
      effects: [],
    },
    '1a6ec06e:node_40': {
      name: '重装骑手',
      buff: '步兵HP +20%',
      effects: [],
    },
    '1a6ec06e:node_41': {
      name: '攻城护盾',
      buff: '全体部队HP +20%',
      effects: [],
    },
    '1a6ec06e:node_42': {
      name: '攻城护甲',
      buff: '弓兵HP +20%',
      effects: [],
    },
    '1a6ec06e:node_43': {
      name: '攻城骑兵',
      buff: '骑兵HP +20%',
      effects: [],
    },
    '1a6ec06e:node_44': {
      name: '占领',
      buff: '攻城胜利加成',
      effects: [],
    },
    'city_defence:node_1': {
      name: '城墙加固',
      buff: '提升城防工事250',
      effects: [],
    },
    'city_defence:node_2': {
      name: '突击队',
      buff: '王座附近的耐久损耗速度降低 20%',
      effects: [],
    },
    'city_defence:node_3': {
      name: '城墙修复',
      buff: '耐久恢复速度 20%',
      effects: [],
    },
    'city_defence:node_4': {
      name: '快速增援',
      buff: '增援时行军速度 20%',
      effects: [],
    },
    'city_defence:node_5': {
      name: '步兵增援攻击',
      buff: '增援盟友时步兵攻击 20%',
      effects: [],
    },
    'city_defence:node_7': {
      name: '骑兵增援攻击',
      buff: '增援盟友时骑兵攻击 20%',
      effects: [],
    },
    'city_defence:node_6': {
      name: '弓兵增援攻击',
      buff: '增援盟友时弓兵攻击 20%',
      effects: [],
    },
    'city_defence:node_8': {
      name: '步兵增援防御',
      buff: '增援盟友时步兵防御 20%',
      effects: [],
    },
    'city_defence:node_10': {
      name: '骑兵增援防御',
      buff: '增援盟友时骑兵防御 20%',
      effects: [],
    },
    'city_defence:node_9': {
      name: '弓兵增援防御',
      buff: '增援盟友时弓兵防御 20%',
      effects: [],
    },
    'city_defence:node_11': {
      name: '快速增援',
      buff: '增援时行军速度 20%',
      effects: [],
    },
    'city_defence:node_12': {
      name: '增援攻击',
      buff: '受到盟友增援时攻击 20%',
      effects: [],
    },
    'city_defence:node_13': {
      name: '增援防御',
      buff: '受到盟友增援时防御 20%',
      effects: [],
    },
    'city_defence:node_14': {
      name: '生生不息',
      buff: '承受敌军的击杀率 -10%',
      effects: [],
    },
    'city_defence:node_15': {
      name: '步兵突围',
      buff: '守城时步兵攻击 20%',
      effects: [],
    },
    'city_defence:node_17': {
      name: '骑兵突围',
      buff: '守城时骑兵攻击 20%',
      effects: [],
    },
    'city_defence:node_16': {
      name: '弓兵突围',
      buff: '守城时弓兵攻击 20%',
      effects: [],
    },
    'city_defence:node_18': {
      name: '守城护盾',
      buff: '守城时步兵防御 20%',
      effects: [],
    },
    'city_defence:node_20': {
      name: '守城骑兵',
      buff: '守城时骑兵防御 20%',
      effects: [],
    },
    'city_defence:node_19': {
      name: '守城护甲',
      buff: '守城时弓兵防御 20%',
      effects: [],
    },
    'city_defence:node_21': {
      name: '团结坚守',
      buff: '获得一个守城军团',
      effects: [],
    },
    'city_defence:node_22': {
      name: '反击',
      buff: '守城队列单位攻击 15%',
      effects: [],
    },
    'city_defence:node_23': {
      name: '坚固要塞',
      buff: '守城队列单位防御 15%',
      effects: [],
    },
    'city_defence:node_24': {
      name: '军团强化',
      buff: '守城队列 HP 15%',
      effects: [],
    },
    'city_defence:node_25': {
      name: '征募扩编',
      buff: '守城队列行军上限增加 15000',
      effects: [],
    },
    'city_defence:node_26': {
      name: '临时病床',
      buff: '医疗帐篷容量 15000',
      effects: [],
    },
    'city_defence:node_27': {
      name: '城墙加固',
      buff: '提升城防工事250',
      effects: [],
    },
    'city_defence:node_28': {
      name: '高级箭塔',
      buff: '将箭塔升级为高级箭塔',
      effects: [],
    },
    'city_defence:node_29': {
      name: '箭塔扩编',
      buff: '高级箭塔能量上限 1000',
      effects: [],
    },
    'city_defence:node_30': {
      name: '箭塔强化',
      buff: '能量恢复速度 40%',
      effects: [],
    },
    'city_defence:node_31': {
      name: '强化箭塔',
      buff: '箭塔攻击 20%',
      effects: [],
    },
    'city_defence:node_32': {
      name: '安全庇护所',
      buff: '战败伤亡降低 20%',
      effects: [],
    },
    'city_defence:node_33': {
      name: '步兵突围',
      buff: '守城时步兵攻击 20%',
      effects: [],
    },
    'city_defence:node_35': {
      name: '骑兵突围',
      buff: '守城时骑兵攻击 20%',
      effects: [],
    },
    'city_defence:node_34': {
      name: '弓兵突围',
      buff: '守城时弓兵攻击 20%',
      effects: [],
    },
    'city_defence:node_36': {
      name: '守城护盾',
      buff: '守城时步兵防御 20%',
      effects: [],
    },
    'city_defence:node_37': {
      name: '守城骑兵',
      buff: '守城时骑兵防御 20%',
      effects: [],
    },
    'city_defence:node_38': {
      name: '守城护甲',
      buff: '守城时弓兵防御 20%',
      effects: [],
    },
    'city_defence:node_39': {
      name: '坚守阵地',
      buff: '数据待确认',
      effects: [],
    },
    'defence_legion:node_1': {
      name: '英雄任命',
      buff: '在守城队列中部署 1 名英雄',
      effects: [],
    },
    'defence_legion:node_2': {
      name: '征募扩编',
      buff: '守城行军兵力增加 10500',
      effects: [],
    },
    'defence_legion:node_3': {
      name: '步兵忠诚',
      buff: '步兵守城队列防御 10%',
      effects: [],
    },
    'defence_legion:node_5': {
      name: '骑兵忠诚',
      buff: '骑兵守城队列防御 10%',
      effects: [],
    },
    'defence_legion:node_4': {
      name: '弓兵忠诚',
      buff: '弓兵守城队列防御 10%',
      effects: [],
    },
    'defence_legion:node_6': {
      name: '步兵突击',
      buff: '步兵守城队列攻击 10%',
      effects: [],
    },
    'defence_legion:node_8': {
      name: '骑兵突击',
      buff: '骑兵守城队列攻击 10%',
      effects: [],
    },
    'defence_legion:node_7': {
      name: '弓兵突击',
      buff: '弓兵守城队列攻击 10%',
      effects: [],
    },
    'defence_legion:node_9': {
      name: '征募扩编',
      buff: '守城行军兵力增加 16500',
      effects: [],
    },
    'defence_legion:node_10': {
      name: '英雄协作',
      buff: '在守城队列中部署 2 名英雄',
      effects: [],
    },
    'defence_legion:node_11': {
      name: '步兵忠诚II',
      buff: '步兵守城队列防御 10%',
      effects: [],
    },
    'defence_legion:node_13': {
      name: '骑兵忠诚II',
      buff: '骑兵守城队列防御 10%',
      effects: [],
    },
    'defence_legion:node_12': {
      name: '弓兵忠诚II',
      buff: '弓兵守城队列防御 10%',
      effects: [],
    },
    'defence_legion:node_14': {
      name: '步兵突击II',
      buff: '步兵守城队列攻击 10%',
      effects: [],
    },
    'defence_legion:node_16': {
      name: '骑兵突击II',
      buff: '骑兵守城队列攻击 10%',
      effects: [],
    },
    'defence_legion:node_15': {
      name: '弓兵突击II',
      buff: '弓兵守城队列攻击 10%',
      effects: [],
    },
    'defence_legion:node_17': {
      name: '英雄指挥',
      buff: '在守城队列中部署 3 名英雄',
      effects: [],
    },
    'defence_legion:node_18': {
      name: '征募扩编II',
      buff: '守城行军兵力增加 25000',
      effects: [],
    },
    'defence_legion:node_19': {
      name: '加固护盾',
      buff: '部队防御加成15%',
      effects: [],
    },
    'defence_legion:node_20': {
      name: '激发潜能',
      buff: '部队攻击加成15%',
      effects: [],
    },
    'defence_legion:node_21': {
      name: '征募扩编III',
      buff: '守城行军兵力增加 26000',
      effects: [],
    },
    'e8704053:node_1': {
      name: '强化进攻技能·步兵',
      buff: '强化步兵基础攻击 +3',
      effects: [],
    },
    'e8704053:node_2': {
      name: '强化进攻技能·骑兵',
      buff: '强化骑兵基础攻击 +3',
      effects: [],
    },
    'e8704053:node_3': {
      name: '强化进攻技能·弓兵',
      buff: '强化弓兵基础攻击 +3',
      effects: [],
    },
    'e8704053:node_4': {
      name: '重击',
      buff: '步兵攻击 +10%',
      effects: [],
    },
    'e8704053:node_5': {
      name: '凶猛冲锋',
      buff: '骑兵攻击 +10%',
      effects: [],
    },
    'e8704053:node_6': {
      name: '强化箭矢',
      buff: '夜战弓兵 +10%',
      effects: [],
    },
    'e8704053:node_7': {
      name: '坚韧打击',
      buff: '全兵种攻击 +5%',
      effects: [],
    },
    'e8704053:node_8': {
      name: '高效训练',
      buff: '强化训练消耗 -10%',
      effects: [],
    },
    'e8704053:node_9': {
      name: '大规模训练',
      buff: '强化训练速度 +40%',
      effects: [],
    },
    'e8704053:node_10': {
      name: '战术打击',
      buff: '全体部队战术攻击 +10%',
      effects: [],
    },
    'e8704053:node_11': {
      name: '强化护甲·步兵',
      buff: '强化步兵基础防御 +5',
      effects: [],
    },
    'e8704053:node_12': {
      name: '强化护甲·骑兵',
      buff: '强化骑兵基础防御 +5',
      effects: [],
    },
    'e8704053:node_13': {
      name: '强化护甲·弓兵',
      buff: '强化弓兵基础防御 +2',
      effects: [],
    },
    'e8704053:node_14': {
      name: '整军护盾',
      buff: '步兵防御 +10%',
      effects: [],
    },
    'e8704053:node_15': {
      name: '强化马铠',
      buff: '骑兵防御 +10%',
      effects: [],
    },
    'e8704053:node_16': {
      name: '强化护臂',
      buff: '弓兵防御 +10%',
      effects: [],
    },
    'e8704053:node_17': {
      name: '大师护甲',
      buff: '全兵种防御 +5%',
      effects: [],
    },
    'e8704053:node_18': {
      name: '祝圣士兵',
      buff: '强化部队训练消耗 -20%',
      effects: [],
    },
    'e8704053:node_19': {
      name: '强效治疗',
      buff: '强化部队治疗速度 +40%',
      effects: [],
    },
    'e8704053:node_20': {
      name: '战术防护',
      buff: '全体部队战术防御 +10%',
      effects: [],
    },
    'e8704053:node_21': {
      name: '步兵征募',
      buff: '步兵训练速度 +40%',
      effects: [],
    },
    'e8704053:node_22': {
      name: '骑兵征募',
      buff: '骑兵训练速度 +40%',
      effects: [],
    },
    'e8704053:node_23': {
      name: '弓兵征募',
      buff: '弓兵训练速度 +40%',
      effects: [],
    },
    'e8704053:node_24': {
      name: '锋利刀刃',
      buff: '强化步兵攻击 +10%',
      effects: [],
    },
    'e8704053:node_25': {
      name: '骑兵剑术',
      buff: '强化骑兵攻击 +10%',
      effects: [],
    },
    'e8704053:node_26': {
      name: '强化箭矢AE',
      buff: '强化弓兵攻击 +10%',
      effects: [],
    },
    'e8704053:node_27': {
      name: '步兵急救',
      buff: '步兵治疗速度 +10%',
      effects: [],
    },
    'e8704053:node_28': {
      name: '兽医',
      buff: '骑兵治疗速度 +10%',
      effects: [],
    },
    'e8704053:node_29': {
      name: '弓兵急救',
      buff: '弓兵治疗速度 +10%',
      effects: [],
    },
    'e8704053:node_30': {
      name: '精工护甲',
      buff: '强化步兵防御 +10%',
      effects: [],
    },
    'e8704053:node_31': {
      name: '强化护甲',
      buff: '强化骑兵防御 +10%',
      effects: [],
    },
    'e8704053:node_32': {
      name: '强化装备',
      buff: '强化弓兵防御 +10%',
      effects: [],
    },
    'e8704053:node_33': {
      name: '戒严令',
      buff: '全体部队HP +6%',
      effects: [],
    },
    'e8704053:node_34': {
      name: '高级医疗',
      buff: '强化部队与普通部队使用相同资源',
      effects: [],
    },
    'b3581c97:node_1': {
      name: '大马士革钢 I',
      buff: '步兵攻击 +10%',
      effects: [],
    },
    'b3581c97:node_2': {
      name: '温血战马I',
      buff: '骑兵攻击 +10%',
      effects: [],
    },
    'b3581c97:node_3': {
      name: '硬化箭矢I',
      buff: '弓兵攻击 +10%',
      effects: [],
    },
    'b3581c97:node_4': {
      name: '全身护甲 I',
      buff: '步兵防御 +10%',
      effects: [],
    },
    'b3581c97:node_5': {
      name: '马镫I',
      buff: '骑兵防御 +10%',
      effects: [],
    },
    'b3581c97:node_6': {
      name: '盾墙 I',
      buff: '弓兵防御 +10%',
      effects: [],
    },
    'b3581c97:node_8': {
      name: '圆形阵 I',
      buff: '战术防御 +5%',
      effects: [],
    },
    'b3581c97:node_11': {
      name: '地道I',
      buff: '战术攻击 +5%',
      effects: [],
    },
    'b3581c97:node_14': {
      name: '狂热I',
      buff: 'HP +5%',
      effects: [],
    },
    'b3581c97:node_17': {
      name: '堡垒I',
      buff: '守城攻击 +5%',
      effects: [],
    },
    'b3581c97:node_20': {
      name: '冲车 I',
      buff: '攻城攻击 +5%',
      effects: [],
    },
    'b3581c97:node_100': {
      name: '地道II',
      buff: '战术攻击 +5%',
      effects: [],
    },
    'b3581c97:node_26': {
      name: '堡垒II',
      buff: '守城攻击 +5%',
      effects: [],
    },
    'b3581c97:node_29': {
      name: '冲车 II',
      buff: '攻城防御 +5%',
      effects: [],
    },
    'b3581c97:node_32': {
      name: '帝国守卫',
      buff: 'T10步兵解锁',
      effects: [],
    },
    'b3581c97:node_35': {
      name: '圆形阵 II',
      buff: '战术防御 +5%',
      effects: [],
    },
    'b3581c97:node_38': {
      name: '全身护甲 II',
      buff: '防御 +10%',
      effects: [],
    },
    'b3581c97:node_41': {
      name: '大马士革钢 II',
      buff: '攻击 +10%',
      effects: [],
    },
    'b3581c97:node_44': {
      name: '堡垒III',
      buff: '守城攻击 +5%',
      effects: [],
    },
    'b3581c97:node_47': {
      name: '冲车 III',
      buff: '攻城攻击 +5%',
      effects: [],
    },
    'b3581c97:node_50': {
      name: '堡垒IV',
      buff: '守城防御 +5%',
      effects: [],
    },
    'b3581c97:node_53': {
      name: '冲车 IV',
      buff: '攻城防御 +5%',
      effects: [],
    },
    'b3581c97:node_56': {
      name: '狂热II',
      buff: 'HP +5%',
      effects: [],
    },
    'b3581c97:node_59': {
      name: '步兵禁卫训练',
      buff: '恢复战力',
      effects: [],
    },
    'b3581c97:node_7': {
      name: '协同攻击 I',
      buff: '战术防御 +5%',
      effects: [],
    },
    'b3581c97:node_10': {
      name: '楔形阵 I',
      buff: '战术攻击 +5%',
      effects: [],
    },
    'b3581c97:node_13': {
      name: '勇气I',
      buff: 'HP +5%',
      effects: [],
    },
    'b3581c97:node_16': {
      name: '武装货车I',
      buff: '守城攻击 +5%',
      effects: [],
    },
    'b3581c97:node_19': {
      name: '壕沟I',
      buff: '攻城攻击 +5%',
      effects: [],
    },
    'b3581c97:node_101': {
      name: '地道II',
      buff: '战术攻击 +5%',
      effects: [],
    },
    'b3581c97:node_25': {
      name: '武装货车II',
      buff: '守城攻击 +5%',
      effects: [],
    },
    'b3581c97:node_28': {
      name: '壕沟II',
      buff: '攻城防御 +5%',
      effects: [],
    },
    'b3581c97:node_31': {
      name: '帝国骑士',
      buff: 'T10骑兵解锁',
      effects: [],
    },
    'b3581c97:node_34': {
      name: '协同攻击 II',
      buff: '战术防御 +5%',
      effects: [],
    },
    'b3581c97:node_37': {
      name: '马镫II',
      buff: '防御 +10%',
      effects: [],
    },
    'b3581c97:node_40': {
      name: '温血战马II',
      buff: '攻击 +10%',
      effects: [],
    },
    'b3581c97:node_43': {
      name: '武装货车III',
      buff: '守城攻击 +5%',
      effects: [],
    },
    'b3581c97:node_46': {
      name: '壕沟III',
      buff: '攻城攻击 +5%',
      effects: [],
    },
    'b3581c97:node_49': {
      name: '武装货车IV',
      buff: '守城防御 +5%',
      effects: [],
    },
    'b3581c97:node_52': {
      name: '壕沟IV',
      buff: '攻城防御 +5%',
      effects: [],
    },
    'b3581c97:node_55': {
      name: '勇气II',
      buff: 'HP +5%',
      effects: [],
    },
    'b3581c97:node_58': {
      name: '骑兵禁卫训练',
      buff: '伤害 +2%',
      effects: [],
    },
    'b3581c97:node_9': {
      name: '掩护射手I',
      buff: '战术防御 +5%',
      effects: [],
    },
    'b3581c97:node_12': {
      name: '轮转射术I',
      buff: '战术攻击 +5%',
      effects: [],
    },
    'b3581c97:node_15': {
      name: '坚韧I',
      buff: 'HP +5%',
      effects: [],
    },
    'b3581c97:node_18': {
      name: '箭塔I',
      buff: '守城攻击 +5%',
      effects: [],
    },
    'b3581c97:node_21': {
      name: '精准射术I',
      buff: '攻城攻击 +5%',
      effects: [],
    },
    'b3581c97:node_102': {
      name: '地道II',
      buff: '战术攻击 +5%',
      effects: [],
    },
    'b3581c97:node_27': {
      name: '箭塔II',
      buff: '守城攻击 +5%',
      effects: [],
    },
    'b3581c97:node_30': {
      name: '精准射术II',
      buff: '攻城防御 +5%',
      effects: [],
    },
    'b3581c97:node_33': {
      name: '帝国射手',
      buff: 'T10弓兵解锁',
      effects: [],
    },
    'b3581c97:node_36': {
      name: '掩护射手II',
      buff: '战术防御 +5%',
      effects: [],
    },
    'b3581c97:node_39': {
      name: '盾墙 II',
      buff: '防御 +10%',
      effects: [],
    },
    'b3581c97:node_42': {
      name: '硬化箭矢II',
      buff: '攻击 +10%',
      effects: [],
    },
    'b3581c97:node_45': {
      name: '箭塔III',
      buff: '守城攻击 +5%',
      effects: [],
    },
    'b3581c97:node_48': {
      name: '精准射术III',
      buff: '攻城攻击 +5%',
      effects: [],
    },
    'b3581c97:node_51': {
      name: '箭塔IV',
      buff: '守城防御 +5%',
      effects: [],
    },
    'b3581c97:node_54': {
      name: '精准射术IV',
      buff: '攻城防御 +5%',
      effects: [],
    },
    'b3581c97:node_57': {
      name: '坚韧II',
      buff: 'HP +5%',
      effects: [],
    },
    'b3581c97:node_60': {
      name: '弓兵禁卫训练',
      buff: '闪避 +3%',
      effects: [],
    },
    '2b149bcb:node_1': {
      name: '勇气守军·步兵',
      buff: '步兵攻城攻击 +5%',
      effects: [],
    },
    '2b149bcb:node_2': {
      name: '勇气守军·骑兵',
      buff: '骑兵攻城攻击 +5%',
      effects: [],
    },
    '2b149bcb:node_3': {
      name: '勇气守军·弓兵',
      buff: '弓兵攻城攻击 +5%',
      effects: [],
    },
    '2b149bcb:node_4': {
      name: '坚韧守军·步兵',
      buff: '步兵攻城防御 +5%',
      effects: [],
    },
    '2b149bcb:node_5': {
      name: '坚韧守军·骑兵',
      buff: '骑兵攻城防御 +5%',
      effects: [],
    },
    '2b149bcb:node_6': {
      name: '坚韧守军·弓兵',
      buff: '弓兵攻城防御 +5%',
      effects: [],
    },
    '2b149bcb:node_7': {
      name: '守军集结I',
      buff: 'T10额外容量 +5m',
      effects: [],
    },
    '2b149bcb:node_8': {
      name: '守军突击',
      buff: '步兵攻击 +5%',
      effects: [],
    },
    '2b149bcb:node_9': {
      name: '守军长枪',
      buff: '骑兵攻击 +5%',
      effects: [],
    },
    '2b149bcb:node_10': {
      name: '守军弓',
      buff: '弓兵攻击 +5%',
      effects: [],
    },
    '2b149bcb:node_11': {
      name: '守军护盾I',
      buff: '步兵防御 +5%',
      effects: [],
    },
    '2b149bcb:node_12': {
      name: '守军护甲I',
      buff: '骑兵防御 +5%',
      effects: [],
    },
    '2b149bcb:node_13': {
      name: '守军阵型I',
      buff: '弓兵防御 +5%',
      effects: [],
    },
    '2b149bcb:node_14': {
      name: '步兵集结',
      buff: 'T10步兵额外容量 +15m',
      effects: [],
    },
    '2b149bcb:node_15': {
      name: '骑兵集结',
      buff: 'T10骑兵额外容量 +15m',
      effects: [],
    },
    '2b149bcb:node_16': {
      name: '弓兵集结',
      buff: 'T10弓兵额外容量 +15m',
      effects: [],
    },
    '2b149bcb:node_17': {
      name: '守军打击',
      buff: '全体部队攻击 +5%',
      effects: [],
    },
    '2b149bcb:node_18': {
      name: '禁卫护盾II',
      buff: '步兵防御 +5%',
      effects: [],
    },
    '2b149bcb:node_19': {
      name: '禁卫护甲II',
      buff: '骑兵防御 +5%',
      effects: [],
    },
    '2b149bcb:node_20': {
      name: '禁卫阵型II',
      buff: '弓兵防御 +5%',
      effects: [],
    },
    '2b149bcb:node_21': {
      name: '守军集结II',
      buff: 'T10额外容量 +30m',
      effects: [],
    },
    '2b149bcb:node_22': {
      name: '伪装守军·步兵',
      buff: '步兵战术防御 +5%',
      effects: [],
    },
    '2b149bcb:node_23': {
      name: '伪装守军·骑兵',
      buff: '骑兵战术防御 +5%',
      effects: [],
    },
    '2b149bcb:node_24': {
      name: '伪装守军·弓兵',
      buff: '弓兵战术防御 +5%',
      effects: [],
    },
    '2b149bcb:node_25': {
      name: '守军突袭·步兵',
      buff: '步兵战术攻击 +5%',
      effects: [],
    },
    '2b149bcb:node_26': {
      name: '守军突袭·骑兵',
      buff: '骑兵战术攻击 +5%',
      effects: [],
    },
    '2b149bcb:node_27': {
      name: '守军突袭·弓兵',
      buff: '弓兵战术攻击 +5%',
      effects: [],
    },
    '2b149bcb:node_28': {
      name: '守军士气·步兵',
      buff: '步兵HP +5%',
      effects: [],
    },
    '2b149bcb:node_29': {
      name: '守军士气·骑兵',
      buff: '骑兵HP +5%',
      effects: [],
    },
    '2b149bcb:node_30': {
      name: '守军士气·弓兵',
      buff: '弓兵HP +5%',
      effects: [],
    },
    '2b149bcb:node_31': {
      name: '守军士气',
      buff: '全体部队HP +5%',
      effects: [],
    },
    '2b149bcb:node_32': {
      name: '守军反击·步兵',
      buff: '步兵守城攻击 +5%',
      effects: [],
    },
    '2b149bcb:node_33': {
      name: '守军反击·骑兵',
      buff: '骑兵守城攻击 +5%',
      effects: [],
    },
    '2b149bcb:node_34': {
      name: '守军反击·弓兵',
      buff: '弓兵守城攻击 +5%',
      effects: [],
    },
    '2b149bcb:node_35': {
      name: '守军决意·步兵',
      buff: '步兵守城防御 +5%',
      effects: [],
    },
    '2b149bcb:node_36': {
      name: '守军决意·骑兵',
      buff: '骑兵守城防御 +5%',
      effects: [],
    },
    '2b149bcb:node_37': {
      name: '守军决意·弓兵',
      buff: '弓兵守城防御 +5%',
      effects: [],
    },
    '2b149bcb:node_38': {
      name: '守军集结III',
      buff: 'T10额外容量 +160m',
      effects: [],
    },
    '3f5ebb34:node_1': {
      name: '武装守军I',
      buff: '步兵防御 +5%',
      effects: [],
    },
    '3f5ebb34:node_2': {
      name: '武装骑兵I',
      buff: '骑兵防御 +5%',
      effects: [],
    },
    '3f5ebb34:node_3': {
      name: '武装弓兵I',
      buff: '弓兵防御 +5%',
      effects: [],
    },
    '3f5ebb34:node_4': {
      name: '步兵利刃I',
      buff: '步兵攻击 +5%',
      effects: [],
    },
    '3f5ebb34:node_5': {
      name: '重装骑手I',
      buff: '骑兵攻击 +5%',
      effects: [],
    },
    '3f5ebb34:node_6': {
      name: '硬化箭矢I',
      buff: '弓兵攻击 +5%',
      effects: [],
    },
    '3f5ebb34:node_7': {
      name: '步兵动员',
      buff: '步兵HP +5%',
      effects: [],
    },
    '3f5ebb34:node_8': {
      name: '骑兵动员',
      buff: '骑兵HP +5%',
      effects: [],
    },
    '3f5ebb34:node_9': {
      name: '弓兵动员',
      buff: '弓兵HP +5%',
      effects: [],
    },
    '3f5ebb34:node_10': {
      name: '全副武装',
      buff: '全兵种防御 +5%',
      effects: [],
    },
    '3f5ebb34:node_11': {
      name: '推进',
      buff: '步兵战术攻击 +5%',
      effects: [],
    },
    '3f5ebb34:node_12': {
      name: '迂回',
      buff: '骑兵战术攻击 +5%',
      effects: [],
    },
    '3f5ebb34:node_13': {
      name: '齐射',
      buff: '弓兵战术攻击 +5%',
      effects: [],
    },
    '3f5ebb34:node_14': {
      name: '步兵利刃II',
      buff: '步兵攻击 +5%',
      effects: [],
    },
    '3f5ebb34:node_15': {
      name: '重装骑手II',
      buff: '骑兵攻击 +5%',
      effects: [],
    },
    '3f5ebb34:node_16': {
      name: '硬化箭矢II',
      buff: '弓兵攻击 +5%',
      effects: [],
    },
    '3f5ebb34:node_17': {
      name: '武装守军II',
      buff: '步兵防御 +5%',
      effects: [],
    },
    '3f5ebb34:node_18': {
      name: '武装骑兵II',
      buff: '骑兵防御 +5%',
      effects: [],
    },
    '3f5ebb34:node_19': {
      name: '武装弓兵II',
      buff: '弓兵防御 +5%',
      effects: [],
    },
    '3f5ebb34:node_20': {
      name: '残暴攻击I',
      buff: '非崇高部队伤害 +4%',
      effects: [],
    },
    '3f5ebb34:node_21': {
      name: '冲锋步兵',
      buff: '步兵攻城攻击 +5%',
      effects: [],
    },
    '3f5ebb34:node_22': {
      name: '冲锋骑兵',
      buff: '骑兵攻城攻击 +5%',
      effects: [],
    },
    '3f5ebb34:node_23': {
      name: '冲锋弓兵',
      buff: '弓兵攻城攻击 +5%',
      effects: [],
    },
    '3f5ebb34:node_24': {
      name: '集结步兵',
      buff: '步兵攻城防御 +5%',
      effects: [],
    },
    '3f5ebb34:node_25': {
      name: '集结骑兵',
      buff: '骑兵攻城防御 +5%',
      effects: [],
    },
    '3f5ebb34:node_26': {
      name: '集结弓兵',
      buff: '弓兵攻城防御 +5%',
      effects: [],
    },
    '3f5ebb34:node_27': {
      name: '激励',
      buff: '全兵种HP +5%',
      effects: [],
    },
    '3f5ebb34:node_28': {
      name: '齐声战吼',
      buff: '步兵防御 +5%',
      effects: [],
    },
    '3f5ebb34:node_29': {
      name: '反制',
      buff: '骑兵防御 +5%',
      effects: [],
    },
    '3f5ebb34:node_30': {
      name: '专注射术',
      buff: '弓兵防御 +5%',
      effects: [],
    },
    '3f5ebb34:node_31': {
      name: '突袭',
      buff: '全兵种攻击 +5%',
      effects: [],
    },
    '3f5ebb34:node_32': {
      name: '硬化利刃',
      buff: '全兵种攻击 +5%',
      effects: [],
    },
    '3f5ebb34:node_33': {
      name: '精锐护甲',
      buff: '全兵种防御 +5%',
      effects: [],
    },
    '3f5ebb34:node_34': {
      name: '战斗架势',
      buff: '全体部队攻城防御 +5%',
      effects: [],
    },
    '3f5ebb34:node_35': {
      name: '防御阵势',
      buff: '全体部队攻城防御 +5%',
      effects: [],
    },
    '3f5ebb34:node_36': {
      name: '残暴攻击II',
      buff: '非崇高部队伤害 +6%',
      effects: [],
    },
    '3ff71437:node_1': {
      name: '坚守阵线 I·步兵',
      buff: '步兵防御 +5%',
      effects: [],
    },
    '3ff71437:node_2': {
      name: '坚守阵线 I·骑兵',
      buff: '骑兵防御 +5%',
      effects: [],
    },
    '3ff71437:node_3': {
      name: '坚守阵线 I·弓兵',
      buff: '弓兵防御 +5%',
      effects: [],
    },
    '3ff71437:node_4': {
      name: '精准打击I步兵',
      buff: '步兵攻击 +5%',
      effects: [],
    },
    '3ff71437:node_5': {
      name: '精准打击I骑兵',
      buff: '骑兵攻击 +5%',
      effects: [],
    },
    '3ff71437:node_6': {
      name: '精准打击I弓兵',
      buff: '弓兵攻击 +5%',
      effects: [],
    },
    '3ff71437:node_7': {
      name: '步兵激励',
      buff: '步兵HP +5%',
      effects: [],
    },
    '3ff71437:node_8': {
      name: '骑兵激励',
      buff: '骑兵HP +5%',
      effects: [],
    },
    '3ff71437:node_9': {
      name: '弓兵激励',
      buff: '弓兵HP +5%',
      effects: [],
    },
    '3ff71437:node_10': {
      name: '深呼吸',
      buff: '全兵种防御 +5%',
      effects: [],
    },
    '3ff71437:node_11': {
      name: '统帅旗帜步兵',
      buff: '步兵战术攻击 +5%',
      effects: [],
    },
    '3ff71437:node_12': {
      name: '统帅旗帜骑兵',
      buff: '骑兵战术攻击 +5%',
      effects: [],
    },
    '3ff71437:node_13': {
      name: '统帅旗帜弓兵',
      buff: '弓兵战术攻击 +5%',
      effects: [],
    },
    '3ff71437:node_14': {
      name: '精准打击II步兵',
      buff: '步兵攻击 +5%',
      effects: [],
    },
    '3ff71437:node_15': {
      name: '精准打击II骑兵',
      buff: '骑兵攻击 +5%',
      effects: [],
    },
    '3ff71437:node_16': {
      name: '精准打击II弓兵',
      buff: '弓兵攻击 +5%',
      effects: [],
    },
    '3ff71437:node_17': {
      name: '坚守阵线 II·步兵',
      buff: '步兵防御 +5%',
      effects: [],
    },
    '3ff71437:node_18': {
      name: '坚守阵线 II·骑兵',
      buff: '骑兵防御 +5%',
      effects: [],
    },
    '3ff71437:node_19': {
      name: '坚守阵线 II·弓兵',
      buff: '弓兵防御 +5%',
      effects: [],
    },
    '3ff71437:node_20': {
      name: '反击I',
      buff: '非崇高部队伤害 +4%',
      effects: [],
    },
    '3ff71437:node_21': {
      name: '攻城号令步兵',
      buff: '步兵攻城攻击 +5%',
      effects: [],
    },
    '3ff71437:node_22': {
      name: '攻城号令骑兵',
      buff: '骑兵攻城攻击 +5%',
      effects: [],
    },
    '3ff71437:node_23': {
      name: '攻城号令弓兵',
      buff: '弓兵攻城攻击 +5%',
      effects: [],
    },
    '3ff71437:node_24': {
      name: '集结步兵',
      buff: '步兵守城攻击 +5%',
      effects: [],
    },
    '3ff71437:node_25': {
      name: '集结骑兵',
      buff: '骑兵守城攻击 +5%',
      effects: [],
    },
    '3ff71437:node_26': {
      name: '集结弓兵',
      buff: '弓兵守城攻击 +5%',
      effects: [],
    },
    '3ff71437:node_27': {
      name: '坚韧',
      buff: '全兵种HP +5%',
      effects: [],
    },
    '3ff71437:node_28': {
      name: '坚守旗帜·步兵战术防御',
      buff: '步兵战术防御 +5%',
      effects: [],
    },
    '3ff71437:node_29': {
      name: '坚守旗帜·骑兵战术防御',
      buff: '骑兵战术防御 +5%',
      effects: [],
    },
    '3ff71437:node_30': {
      name: '坚守旗帜·弓兵战术防御',
      buff: '弓兵战术防御 +5%',
      effects: [],
    },
    '3ff71437:node_31': {
      name: '致命猛击',
      buff: '全兵种攻击 +5%',
      effects: [],
    },
    '3ff71437:node_32': {
      name: '武器强化',
      buff: '全兵种攻击 +5%',
      effects: [],
    },
    '3ff71437:node_33': {
      name: '强化护甲',
      buff: '全兵种防御 +5%',
      effects: [],
    },
    '3ff71437:node_34': {
      name: '战斗架势II',
      buff: '全体部队攻城防御 +5%',
      effects: [],
    },
    '3ff71437:node_35': {
      name: '防御架势II',
      buff: '全体部队攻城防御 +5%',
      effects: [],
    },
    '3ff71437:node_36': {
      name: '反击II',
      buff: '守城时非崇高部队伤害 +6%',
      effects: [],
    },
    'art_of_war_command:node_1': {
      name: '谋略剑术',
      buff: '步兵守城攻击 +5%',
      effects: [],
    },
    'art_of_war_command:node_2': {
      name: '谋略骑术',
      buff: '骑兵守城攻击 +5%',
      effects: [],
    },
    'art_of_war_command:node_3': {
      name: '谋略弓术',
      buff: '弓兵守城攻击 +5%',
      effects: [],
    },
    'art_of_war_command:node_4': {
      name: '忠诚剑术',
      buff: '步兵守城防御 +5%',
      effects: [],
    },
    'art_of_war_command:node_5': {
      name: '忠诚骑术',
      buff: '骑兵守城防御 +5%',
      effects: [],
    },
    'art_of_war_command:node_6': {
      name: '忠诚弓术',
      buff: '弓兵守城防御 +5%',
      effects: [],
    },
    'art_of_war_command:node_7': {
      name: '生命之泉 I',
      buff: '全兵种HP +5%',
      effects: [],
    },
    'art_of_war_command:node_8': {
      name: '无畏剑术',
      buff: '步兵攻城攻击 +5%',
      effects: [],
    },
    'art_of_war_command:node_9': {
      name: '无畏骑术',
      buff: '骑兵攻城攻击 +5%',
      effects: [],
    },
    'art_of_war_command:node_10': {
      name: '无畏弓术',
      buff: '弓兵攻城攻击 +5%',
      effects: [],
    },
    'art_of_war_command:node_11': {
      name: '坚定剑术',
      buff: '步兵攻城防御 +5%',
      effects: [],
    },
    'art_of_war_command:node_12': {
      name: '坚定骑术',
      buff: '骑兵攻城防御 +5%',
      effects: [],
    },
    'art_of_war_command:node_13': {
      name: '坚定弓术',
      buff: '弓兵攻城防御 +5%',
      effects: [],
    },
    'art_of_war_command:node_14': {
      name: '势不可挡I',
      buff: '全兵种攻击 +5%',
      effects: [],
    },
    'art_of_war_command:node_15': {
      name: '潜行剑术I',
      buff: '步兵防御 +5%',
      effects: [],
    },
    'art_of_war_command:node_16': {
      name: '潜行骑术I',
      buff: '骑兵防御 +5%',
      effects: [],
    },
    'art_of_war_command:node_17': {
      name: '潜行弓术I',
      buff: '弓兵防御 +5%',
      effects: [],
    },
    'art_of_war_command:node_18': {
      name: '强力剑术I',
      buff: '步兵HP +5%',
      effects: [],
    },
    'art_of_war_command:node_19': {
      name: '强力骑术I',
      buff: '骑兵HP +5%',
      effects: [],
    },
    'art_of_war_command:node_20': {
      name: '强力弓术I',
      buff: '弓兵HP +5%',
      effects: [],
    },
    'art_of_war_command:node_21': {
      name: '势不可挡II',
      buff: '全兵种攻击 +5%',
      effects: [],
    },
    'art_of_war_command:node_22': {
      name: '潜行剑术II',
      buff: '步兵防御 +5%',
      effects: [],
    },
    'art_of_war_command:node_23': {
      name: '潜行骑术II',
      buff: '骑兵防御 +5%',
      effects: [],
    },
    'art_of_war_command:node_24': {
      name: '潜行弓术II',
      buff: '弓兵防御 +5%',
      effects: [],
    },
    'art_of_war_command:node_25': {
      name: '突围剑术',
      buff: '步兵攻城攻击 +5%',
      effects: [],
    },
    'art_of_war_command:node_26': {
      name: '突围骑术',
      buff: '骑兵攻城攻击 +5%',
      effects: [],
    },
    'art_of_war_command:node_27': {
      name: '突围弓术',
      buff: '弓兵攻城攻击 +5%',
      effects: [],
    },
    'art_of_war_command:node_28': {
      name: '强力剑术II',
      buff: '步兵HP +5%',
      effects: [],
    },
    'art_of_war_command:node_29': {
      name: '强力骑术II',
      buff: '骑兵HP +5%',
      effects: [],
    },
    'art_of_war_command:node_30': {
      name: '强力弓术II',
      buff: '弓兵HP +5%',
      effects: [],
    },
    'art_of_war_command:node_31': {
      name: '隐藏天赋',
      buff: '全体部队防御 +10%',
      effects: [],
    },
    'art_of_war_command:node_32': {
      name: '谋略指挥·步兵',
      buff: '步兵战术防御 +5%',
      effects: [],
    },
    'art_of_war_command:node_33': {
      name: '谋略指挥·骑兵',
      buff: '骑兵战术防御 +5%',
      effects: [],
    },
    'art_of_war_command:node_34': {
      name: '谋略指挥·弓兵',
      buff: '弓兵战术防御 +5%',
      effects: [],
    },
    'art_of_war_command:node_35': {
      name: '攻城号令·步兵',
      buff: '步兵战术攻击 +5%',
      effects: [],
    },
    'art_of_war_command:node_36': {
      name: '攻城号令·骑兵',
      buff: '骑兵战术攻击 +5%',
      effects: [],
    },
    'art_of_war_command:node_37': {
      name: '攻城号令·弓兵',
      buff: '弓兵战术攻击 +5%',
      effects: [],
    },
    'art_of_war_command:node_38': {
      name: '生命之泉 II',
      buff: '全兵种HP +10%',
      effects: [],
    },
    '77db4a78:node_1': {
      name: '强力打击',
      buff: '步兵攻击 20%',
      effects: [],
    },
    '77db4a78:node_3': {
      name: '凶猛冲锋',
      buff: '骑兵攻击 20%',
      effects: [],
    },
    '77db4a78:node_2': {
      name: '强化箭矢',
      buff: '弓兵攻击 20%',
      effects: [],
    },
    '77db4a78:node_4': {
      name: '势不可挡',
      buff: '全体单位攻击 10%',
      effects: [],
    },
    '77db4a78:node_5': {
      name: '大师护甲',
      buff: '步兵防御 20%',
      effects: [],
    },
    '77db4a78:node_7': {
      name: '强化马铠',
      buff: '骑兵防御 20%',
      effects: [],
    },
    '77db4a78:node_6': {
      name: '强化护臂',
      buff: '弓兵防御 20%',
      effects: [],
    },
    '77db4a78:node_8': {
      name: '全副武装',
      buff: '全体单位防御 10%',
      effects: [],
    },
    '77db4a78:node_9': {
      name: '体魄强化',
      buff: '步兵HP 10%',
      effects: [],
    },
    '77db4a78:node_10': {
      name: '法力药剂',
      buff: '骑兵HP 10%',
      effects: [],
    },
    '77db4a78:node_11': {
      name: '重装掠袭者',
      buff: '弓兵HP 10%',
      effects: [],
    },
    '77db4a78:node_12': {
      name: '全金属护甲',
      buff: '全体HP 5%',
      effects: [],
    },
    '77db4a78:node_13_F': {
      name: '崇高步兵',
      buff: 'T10步兵解锁',
      effects: [],
    },
    '77db4a78:node_13_A': {
      name: '崇高弓兵',
      buff: 'T10弓兵解锁',
      effects: [],
    },
    '77db4a78:node_13_C': {
      name: 'T10解锁',
      buff: 'T10解锁',
      effects: [],
    },
    '77db4a78:node_14_F': {
      name: '致命突击',
      buff: '崇高步兵攻击 9%',
      effects: [],
    },
    '77db4a78:node_15_F': {
      name: '优质护甲',
      buff: '崇高步兵防御 6%',
      effects: [],
    },
    '77db4a78:node_16_F': {
      name: '高级护甲',
      buff: '崇高步兵HP 3%',
      effects: [],
    },
    '77db4a78:node_17_F': {
      name: '强力打击 II',
      buff: '步兵攻击 30%',
      effects: [],
    },
    '77db4a78:node_18_F': {
      name: '大师护甲 II',
      buff: '步兵防御 30%',
      effects: [],
    },
    '77db4a78:node_19_F': {
      name: '体魄强化II',
      buff: '步兵HP 15%',
      effects: [],
    },
    '77db4a78:node_20_F': {
      name: '崇高步兵上限',
      buff: '崇高上限 +200k',
      effects: [],
    },
    '77db4a78:node_21_F': {
      name: '障碍训练',
      buff: '崇高步兵训练速度 +50%',
      effects: [],
    },
    '77db4a78:node_22_F': {
      name: '大规模生产',
      buff: '崇高步兵训练消耗 -50%',
      effects: [],
    },
    '77db4a78:node_23_F': {
      name: '后备药剂',
      buff: '崇高步兵治疗速度 +50%',
      effects: [],
    },
    '77db4a78:node_24_F': {
      name: '奇迹治疗',
      buff: '崇高步兵治疗消耗 -50%',
      effects: [],
    },
    '77db4a78:node_25_F': {
      name: '崇高步兵上限II',
      buff: '崇高上限 +800k',
      effects: [],
    },
    '77db4a78:node_26_F': {
      name: '强力打击 III',
      buff: '步兵攻击 42%',
      effects: [],
    },
    '77db4a78:node_27_F': {
      name: '大师护甲 III',
      buff: '步兵防御 40%',
      effects: [],
    },
    '77db4a78:node_28_F': {
      name: '体魄强化III',
      buff: '步兵HP 20%',
      effects: [],
    },
    '77db4a78:node_29_F': {
      name: '勇猛突击',
      buff: '崇高步兵伤害 20%',
      effects: [],
    },
    '77db4a78:node_14_C': {
      name: '致命突击',
      buff: '崇高骑兵攻击 9%',
      effects: [],
    },
    '77db4a78:node_15_C': {
      name: '优质马铠',
      buff: '崇高骑兵防御 6%',
      effects: [],
    },
    '77db4a78:node_16_C': {
      name: '高级马铠',
      buff: '崇高骑兵HP 3%',
      effects: [],
    },
    '77db4a78:node_17_C': {
      name: '凶猛冲锋II',
      buff: '骑兵攻击 30%',
      effects: [],
    },
    '77db4a78:node_18_C': {
      name: '强化马铠II',
      buff: '骑兵防御 30%',
      effects: [],
    },
    '77db4a78:node_19_C': {
      name: '重装掠袭者II',
      buff: '骑兵HP 15%',
      effects: [],
    },
    '77db4a78:node_20_C': {
      name: '混乱掠袭者',
      buff: '崇高上限 +200k',
      effects: [],
    },
    '77db4a78:node_21_C': {
      name: '障碍训练',
      buff: '崇高骑兵训练速度 +50%',
      effects: [],
    },
    '77db4a78:node_22_C': {
      name: '大规模生产',
      buff: '崇高骑兵训练消耗 -50%',
      effects: [],
    },
    '77db4a78:node_23_C': {
      name: '后备药剂',
      buff: '崇高骑兵治疗速度 +50%',
      effects: [],
    },
    '77db4a78:node_24_C': {
      name: '奇迹治疗',
      buff: '崇高骑兵治疗消耗 -50%',
      effects: [],
    },
    '77db4a78:node_25_C': {
      name: '混乱掠袭者II',
      buff: '崇高上限 +800k',
      effects: [],
    },
    '77db4a78:node_26_C': {
      name: '凶猛冲锋III',
      buff: '骑兵攻击 42%',
      effects: [],
    },
    '77db4a78:node_27_C': {
      name: '强化马铠III',
      buff: '骑兵防御 40%',
      effects: [],
    },
    '77db4a78:node_28_C': {
      name: '重装掠袭者III',
      buff: '骑兵HP 20%',
      effects: [],
    },
    '77db4a78:node_29_C': {
      name: '勇猛突击',
      buff: '崇高骑兵伤害 20%',
      effects: [],
    },
    '77db4a78:node_14_A': {
      name: '致命突击',
      buff: '崇高弓兵攻击 9%',
      effects: [],
    },
    '77db4a78:node_15_A': {
      name: '优质护甲',
      buff: '崇高弓兵防御 6%',
      effects: [],
    },
    '77db4a78:node_16_A': {
      name: '高级护甲',
      buff: '崇高弓兵HP 3%',
      effects: [],
    },
    '77db4a78:node_17_A': {
      name: '强化箭矢II',
      buff: '弓兵攻击 30%',
      effects: [],
    },
    '77db4a78:node_18_A': {
      name: '强化护臂 II',
      buff: '弓兵防御 30%',
      effects: [],
    },
    '77db4a78:node_19_A': {
      name: '重装掠袭者II',
      buff: '弓兵HP 15%',
      effects: [],
    },
    '77db4a78:node_20_A': {
      name: '崇高弓兵上限',
      buff: '崇高上限 +200k',
      effects: [],
    },
    '77db4a78:node_21_A': {
      name: '障碍训练',
      buff: '崇高弓兵训练速度 +50%',
      effects: [],
    },
    '77db4a78:node_22_A': {
      name: '大规模生产',
      buff: '崇高弓兵训练消耗 -50%',
      effects: [],
    },
    '77db4a78:node_23_A': {
      name: '后备药剂',
      buff: '崇高弓兵治疗速度 +50%',
      effects: [],
    },
    '77db4a78:node_24_A': {
      name: '奇迹治疗',
      buff: '崇高弓兵治疗消耗 -50%',
      effects: [],
    },
    '77db4a78:node_25_A': {
      name: '崇高弓兵上限II',
      buff: '崇高上限 +800k',
      effects: [],
    },
    '77db4a78:node_26_A': {
      name: '强化箭矢III',
      buff: '弓兵攻击 42%',
      effects: [],
    },
    '77db4a78:node_27_A': {
      name: '强化护臂 III',
      buff: '弓兵防御 40%',
      effects: [],
    },
    '77db4a78:node_28_A': {
      name: '重装掠袭者III',
      buff: '弓兵HP 20%',
      effects: [],
    },
    '77db4a78:node_29_A': {
      name: '勇猛突击',
      buff: '崇高弓兵伤害 20%',
      effects: [],
    },
    '24ffc526:node_1': {
      name: '骑兵·勇气满盈',
      buff: '前排攻击 10%',
      effects: [],
    },
    '24ffc526:node_2': {
      name: '坚韧·骑兵',
      buff: '前排防御 10%',
      effects: [],
    },
    '24ffc526:node_3': {
      name: '弓兵·勇气满盈',
      buff: '前排攻击 10%',
      effects: [],
    },
    '24ffc526:node_4': {
      name: '坚韧·弓兵',
      buff: '前排防御 10%',
      effects: [],
    },
    '24ffc526:node_5': {
      name: '步兵·勇气满盈',
      buff: '前排攻击 10%',
      effects: [],
    },
    '24ffc526:node_6': {
      name: '坚韧·步兵',
      buff: '前排防御 10%',
      effects: [],
    },
    '24ffc526:node_7': {
      name: '前排防御',
      buff: '前排所有单位战术防御 10%',
      effects: [],
    },
    '24ffc526:node_8': {
      name: '步兵突袭',
      buff: '步兵行军速度 100%',
      effects: [],
    },
    '24ffc526:node_9': {
      name: '弓兵突袭',
      buff: '弓兵行军速度 100%',
      effects: [],
    },
    '24ffc526:node_10': {
      name: '冲锋前进',
      buff: '纯骑兵军团的骑兵攻击 10%',
      effects: [],
    },
    '24ffc526:node_11': {
      name: '推进阵型·骑兵',
      buff: '中排攻击 10%',
      effects: [],
    },
    '24ffc526:node_12': {
      name: '推进阵型·弓兵',
      buff: '中排攻击 10%',
      effects: [],
    },
    '24ffc526:node_13': {
      name: '推进阵型·步兵',
      buff: '中排攻击 10%',
      effects: [],
    },
    '24ffc526:node_14': {
      name: '侧翼防护·骑兵',
      buff: '中排防御 10%',
      effects: [],
    },
    '24ffc526:node_15': {
      name: '侧翼防护·弓兵',
      buff: '中排防御 10%',
      effects: [],
    },
    '24ffc526:node_16': {
      name: '侧翼防护·步兵',
      buff: '中排防御 10%',
      effects: [],
    },
    '24ffc526:node_17': {
      name: '中央战力',
      buff: '中排所有单位战术攻击 10%',
      effects: [],
    },
    '24ffc526:node_18': {
      name: '骑兵主力',
      buff: '后排攻击 10%',
      effects: [],
    },
    '24ffc526:node_19': {
      name: '弓兵主力',
      buff: '后排攻击 10%',
      effects: [],
    },
    '24ffc526:node_20': {
      name: '步兵主力',
      buff: '后排攻击 10%',
      effects: [],
    },
    '24ffc526:node_21': {
      name: '秘密战术·骑兵',
      buff: '后排防御 10%',
      effects: [],
    },
    '24ffc526:node_22': {
      name: '秘密战术·弓兵',
      buff: '后排防御 10%',
      effects: [],
    },
    '24ffc526:node_23': {
      name: '秘密战术·步兵',
      buff: '后排防御 10%',
      effects: [],
    },
    '24ffc526:node_24': {
      name: '统帅战力',
      buff: '后排所有单位战术攻击 10%',
      effects: [],
    },
    '24ffc526:node_25': {
      name: '集中攻击',
      buff: '后排攻击 10%',
      effects: [],
    },
    '24ffc526:node_26': {
      name: '协同机动',
      buff: '中排攻击 10%',
      effects: [],
    },
    '24ffc526:node_27': {
      name: '推进前线',
      buff: '前排攻击 10%',
      effects: [],
    },
    '24ffc526:node_28': {
      name: '转阵防御',
      buff: '中排防御 10%',
      effects: [],
    },
    '24ffc526:node_29': {
      name: '"掩护我！"',
      buff: '后排防御 10%',
      effects: [],
    },
    '24ffc526:node_30': {
      name: '冲击抵抗',
      buff: '前排防御 10%',
      effects: [],
    },
    '24ffc526:node_31': {
      name: '近身战斗',
      buff: '前排 HP +10%',
      effects: [],
    },
    '24ffc526:node_32': {
      name: '中央战力',
      buff: '中排战术攻击 20%',
      effects: [],
    },
    '24ffc526:node_33': {
      name: '中央防御',
      buff: '中排战术防御 20%',
      effects: [],
    },
    '24ffc526:node_34': {
      name: '坚韧打击',
      buff: '后排伤害 5%',
      effects: [],
    },
    '2c49bd2a:node_1': {
      name: '谋略攻城',
      buff: '守城时战术攻击 +10%',
      effects: [],
    },
    '2c49bd2a:node_2': {
      name: '稳固步兵防御',
      buff: '非攻城战中步兵防御 +5%',
      effects: [],
    },
    '2c49bd2a:node_3': {
      name: '稳固骑兵防御',
      buff: '非攻城战中骑兵防御 +5%',
      effects: [],
    },
    '2c49bd2a:node_4': {
      name: '稳固弓兵防御',
      buff: '非攻城战中弓兵防御 +5%',
      effects: [],
    },
    '2c49bd2a:node_5': {
      name: '城门破击步兵',
      buff: '步兵攻城攻击 +5%',
      effects: [],
    },
    '2c49bd2a:node_6': {
      name: '城堡强袭骑兵',
      buff: '骑兵攻城攻击 +5%',
      effects: [],
    },
    '2c49bd2a:node_7': {
      name: '敌军麻痹弓兵',
      buff: '弓兵攻城攻击 +5%',
      effects: [],
    },
    '2c49bd2a:node_8': {
      name: '出奇制胜',
      buff: '非攻城战中战术攻击 +10%',
      effects: [],
    },
    '2c49bd2a:node_9': {
      name: '步兵防御阵线',
      buff: '步兵攻城防御 +5%',
      effects: [],
    },
    '2c49bd2a:node_10': {
      name: '壁垒骑兵',
      buff: '骑兵攻城防御 +5%',
      effects: [],
    },
    '2c49bd2a:node_11': {
      name: '弓兵后排防御',
      buff: '弓兵攻城防御 +5%',
      effects: [],
    },
    '2c49bd2a:node_12': {
      name: '城墙破阵',
      buff: '守城时步兵攻击 +5%',
      effects: [],
    },
    '2c49bd2a:node_13': {
      name: '骑兵冲锋',
      buff: '守城时骑兵攻击 +5%',
      effects: [],
    },
    '2c49bd2a:node_14': {
      name: '箭雨',
      buff: '守城时弓兵攻击 +5%',
      effects: [],
    },
    '2c49bd2a:node_15': {
      name: '攻城计策',
      buff: '攻城时战术攻击 +10%',
      effects: [],
    },
    '2c49bd2a:node_16': {
      name: '城墙守卫',
      buff: '守城时步兵攻击 +5%',
      effects: [],
    },
    '2c49bd2a:node_17': {
      name: '骑兵黄金盾',
      buff: '守城时骑兵攻击 +5%',
      effects: [],
    },
    '2c49bd2a:node_18': {
      name: '弓兵箭塔防御',
      buff: '守城时弓兵攻击 +5%',
      effects: [],
    },
    '2c49bd2a:node_19': {
      name: '步兵突围',
      buff: '非攻城战中步兵攻击 +5%',
      effects: [],
    },
    '2c49bd2a:node_20': {
      name: '骑兵突袭',
      buff: '非攻城战中骑兵攻击 +5%',
      effects: [],
    },
    '2c49bd2a:node_21': {
      name: '弓兵远程打击',
      buff: '非攻城战中弓兵攻击 +5%',
      effects: [],
    },
    '2c49bd2a:node_22': {
      name: '满编部队',
      buff: '部队战力高于90%时，伤害减免+5%',
      effects: [],
    },
    '2c49bd2a:node_23': {
      name: '突袭步兵',
      buff: '步兵攻击+5%',
      effects: [],
    },
    '2c49bd2a:node_24': {
      name: '进军步兵',
      buff: '步兵防御+5%',
      effects: [],
    },
    '2c49bd2a:node_25': {
      name: '骑兵突袭',
      buff: '骑兵攻击+5%',
      effects: [],
    },
    '2c49bd2a:node_26': {
      name: '守堡骑兵',
      buff: '骑兵防御+5%',
      effects: [],
    },
    '2c49bd2a:node_27': {
      name: '弓兵射术',
      buff: '弓兵攻击+5%',
      effects: [],
    },
    '2c49bd2a:node_28': {
      name: '守城弓兵',
      buff: '弓兵防御+5%',
      effects: [],
    },
    '2c49bd2a:node_29': {
      name: '洞察增援',
      buff: '战术防御+10%',
      effects: [],
    },
    '2c49bd2a:node_30': {
      name: '步兵快速操练',
      buff: '步兵训练速度+10%',
      effects: [],
    },
    '2c49bd2a:node_31': {
      name: '骑兵快速操练',
      buff: '骑兵训练速度+10%',
      effects: [],
    },
    '2c49bd2a:node_32': {
      name: '弓兵快速操练',
      buff: '弓兵训练速度+10%',
      effects: [],
    },
    '2c49bd2a:node_33': {
      name: '进军步兵',
      buff: '步兵防御+10%',
      effects: [],
    },
    '2c49bd2a:node_34': {
      name: '守堡骑兵',
      buff: '骑兵防御+10%',
      effects: [],
    },
    '2c49bd2a:node_35': {
      name: '守城弓兵',
      buff: '弓兵防御+10%',
      effects: [],
    },
    '2c49bd2a:node_36': {
      name: '突袭步兵',
      buff: '步兵攻击+10%',
      effects: [],
    },
    '2c49bd2a:node_37': {
      name: '骑兵突袭',
      buff: '骑兵攻击+10%',
      effects: [],
    },
    '2c49bd2a:node_38': {
      name: '弓兵射术',
      buff: '弓兵攻击+10%',
      effects: [],
    },
    '2c49bd2a:node_39': {
      name: '意志坚定的士兵',
      buff: '全体单位HP+10%',
      effects: [],
    },
    '2c49bd2a:node_40': {
      name: '步兵强力训练',
      buff: '10 级步兵训练消耗降低幅度 +10%',
      effects: [],
    },
    '2c49bd2a:node_41': {
      name: '骑兵强力训练',
      buff: '10 级骑兵训练消耗降低幅度 +10%',
      effects: [],
    },
    '2c49bd2a:node_42': {
      name: '弓兵强力训练',
      buff: '10 级弓兵训练消耗降低幅度 +10%',
      effects: [],
    },
    '2c49bd2a:node_43': {
      name: '果断进军',
      buff: '步兵造成的伤害 +5%',
      effects: [],
    },
    '2c49bd2a:node_44': {
      name: '破阵骑兵',
      buff: '步兵造成的伤害 +5%',
      effects: [],
    },
    '2c49bd2a:node_45': {
      name: '箭如雨下',
      buff: '步兵造成的伤害 +5%',
      effects: [],
    },
    'fc190e81:node_1': {
      name: '生命之泉 I',
      buff: '全体部队HP +8%',
      effects: [],
    },
    'fc190e81:node_2': {
      name: '推进前线',
      buff: '前排攻击 +8%',
      effects: [],
    },
    'fc190e81:node_3': {
      name: '协同机动',
      buff: '中排攻击 +8%',
      effects: [],
    },
    'fc190e81:node_4': {
      name: '集中攻击',
      buff: '后排攻击 +8%',
      effects: [],
    },
    'fc190e81:node_5': {
      name: '武装守军I',
      buff: '步兵防御 +8%',
      effects: [],
    },
    'fc190e81:node_6': {
      name: '武装弓兵I',
      buff: '弓兵防御 +8%',
      effects: [],
    },
    'fc190e81:node_7': {
      name: '武装骑兵I',
      buff: '骑兵防御 +8%',
      effects: [],
    },
    'fc190e81:node_8': {
      name: '龙血护盾 I',
      buff: '受到伤害 -5%',
      effects: [],
    },
    'fc190e81:node_9': {
      name: '冲击抵抗',
      buff: '前排防御 +8%',
      effects: [],
    },
    'fc190e81:node_10': {
      name: '转阵防御',
      buff: '中排防御 +8%',
      effects: [],
    },
    'fc190e81:node_11': {
      name: '掩护我',
      buff: '后排防御 +8%',
      effects: [],
    },
    'fc190e81:node_12': {
      name: '步兵利刃I',
      buff: '步兵攻击 +8%',
      effects: [],
    },
    'fc190e81:node_13': {
      name: '硬化箭矢I',
      buff: '弓兵攻击 +8%',
      effects: [],
    },
    'fc190e81:node_14': {
      name: '重装骑手I',
      buff: '骑兵攻击 +8%',
      effects: [],
    },
    'fc190e81:node_15': {
      name: '强化训练',
      buff: '训练速度 +5%',
      effects: [],
    },
    'fc190e81:node_16': {
      name: '强力剑术I',
      buff: '步兵HP +10%',
      effects: [],
    },
    'fc190e81:node_17': {
      name: '强力弓术I',
      buff: '弓兵HP +10%',
      effects: [],
    },
    'fc190e81:node_18': {
      name: '强力骑术I',
      buff: '骑兵HP +10%',
      effects: [],
    },
    'fc190e81:node_19': {
      name: '龙血之力',
      buff: '造成伤害 +5%',
      effects: [],
    },
    'fc190e81:node_20': {
      name: '冲锋',
      buff: '攻城攻击 +12%',
      effects: [],
    },
    'fc190e81:node_21': {
      name: '集结',
      buff: '守城攻击 +12%',
      effects: [],
    },
    'fc190e81:node_22': {
      name: '游击战',
      buff: '非攻城战攻击 +12%',
      effects: [],
    },
    'fc190e81:node_23': {
      name: '战斗阵势',
      buff: '攻城防御 +12%',
      effects: [],
    },
    'fc190e81:node_24': {
      name: '防御阵势',
      buff: '守城防御 +12%',
      effects: [],
    },
    'fc190e81:node_25': {
      name: '角斗士阵势',
      buff: '非攻城战防御 +12%',
      effects: [],
    },
    'fc190e81:node_26': {
      name: '生命之泉 II',
      buff: '全体部队HP +12%',
      effects: [],
    },
    'fc190e81:node_27': {
      name: '推进前线II',
      buff: '前排攻击 +12%',
      effects: [],
    },
    'fc190e81:node_28': {
      name: '冲击防护II',
      buff: '前排防御 +12%',
      effects: [],
    },
    'fc190e81:node_29': {
      name: '协同机动II',
      buff: '中排攻击 +12%',
      effects: [],
    },
    'fc190e81:node_30': {
      name: '转阵防御II',
      buff: '中排防御 +12%',
      effects: [],
    },
    'fc190e81:node_31': {
      name: '集中攻击II',
      buff: '后排攻击 +12%',
      effects: [],
    },
    'fc190e81:node_32': {
      name: '掩护我 II',
      buff: '后排防御 +12%',
      effects: [],
    },
    'fc190e81:node_33': {
      name: '龙血治疗',
      buff: '治疗速度 +5%',
      effects: [],
    },
    'fc190e81:node_34': {
      name: '步兵利刃II',
      buff: '步兵攻击 +20%',
      effects: [],
    },
    'fc190e81:node_35': {
      name: '硬化箭矢II',
      buff: '弓兵攻击 +20%',
      effects: [],
    },
    'fc190e81:node_36': {
      name: '重装骑手II',
      buff: '骑兵攻击 +20%',
      effects: [],
    },
    'fc190e81:node_37': {
      name: '武装守军II',
      buff: '步兵防御 +20%',
      effects: [],
    },
    'fc190e81:node_38': {
      name: '武装弓兵II',
      buff: '弓兵防御 +20%',
      effects: [],
    },
    'fc190e81:node_39': {
      name: '武装骑兵II',
      buff: '骑兵防御 +20%',
      effects: [],
    },
    'fc190e81:node_40': {
      name: '龙血护盾 II',
      buff: '受到伤害 -5%',
      effects: [],
    },
    'fc190e81:node_41': {
      name: '强力剑术II',
      buff: '步兵HP +30%',
      effects: [],
    },
    'fc190e81:node_42': {
      name: '强力弓术II',
      buff: '弓兵HP +30%',
      effects: [],
    },
    'fc190e81:node_43': {
      name: '强力骑术II',
      buff: '骑兵HP +30%',
      effects: [],
    },
    '19ae0569:node_1': {
      name: '周密救援',
      buff: '增援时盟友防御 +10%',
      effects: [],
    },
    '19ae0569:node_2': {
      name: '训练大师',
      buff: '已训练部队 +3000',
      effects: [],
    },
    '19ae0569:node_3': {
      name: '联合防御',
      buff: '受到增援时自身防御 +10%',
      effects: [],
    },
    '19ae0569:node_4': {
      name: '完美协作',
      buff: '增援时自身攻击 +10%',
      effects: [],
    },
    '19ae0569:node_5': {
      name: '强化大师',
      buff: '强化 +20000',
      effects: [],
    },
    '19ae0569:node_6': {
      name: '反击',
      buff: '受到增援时盟友攻击 +10%',
      effects: [],
    },
    '19ae0569:node_7': {
      name: '步兵狂热I',
      buff: '步兵HP +10%',
      effects: [],
    },
    '19ae0569:node_8': {
      name: '骑兵作战 I',
      buff: '骑兵攻击 +10%',
      effects: [],
    },
    '19ae0569:node_9': {
      name: '弓兵狂热I',
      buff: '弓兵HP +10%',
      effects: [],
    },
    '19ae0569:node_10': {
      name: '野战医院',
      buff: '医院容量 +150k',
      effects: [],
    },
    '19ae0569:node_11': {
      name: '步兵狂热II',
      buff: '步兵HP +10%',
      effects: [],
    },
    '19ae0569:node_12': {
      name: '骑兵作战 II',
      buff: '骑兵攻击 +10%',
      effects: [],
    },
    '19ae0569:node_13': {
      name: '弓兵狂热II',
      buff: '弓兵HP +10%',
      effects: [],
    },
    '19ae0569:node_14': {
      name: '步兵扩编I',
      buff: '崇高步兵容量 +500k',
      effects: [],
    },
    '19ae0569:node_15': {
      name: '骑兵扩编I',
      buff: '崇高骑兵容量 +500k',
      effects: [],
    },
    '19ae0569:node_16': {
      name: '弓兵扩编I',
      buff: '崇高弓兵容量 +500k',
      effects: [],
    },
    '19ae0569:node_17': {
      name: '护甲精炼',
      buff: '防御 +5%',
      effects: [],
    },
    '19ae0569:node_18': {
      name: '步兵突袭',
      buff: '步兵战术攻击 +10%',
      effects: [],
    },
    '19ae0569:node_19': {
      name: '骑兵突击',
      buff: '骑兵战术攻击 +10%',
      effects: [],
    },
    '19ae0569:node_20': {
      name: '弓兵射术',
      buff: '弓兵战术攻击 +10%',
      effects: [],
    },
    '19ae0569:node_21': {
      name: '步兵护盾',
      buff: '步兵战术防御 +10%',
      effects: [],
    },
    '19ae0569:node_22': {
      name: '后排骑兵',
      buff: '骑兵战术防御 +10%',
      effects: [],
    },
    '19ae0569:node_23': {
      name: '弓兵掩护',
      buff: '弓兵战术防御 +10%',
      effects: [],
    },
    '19ae0569:node_24': {
      name: '步兵扩编II',
      buff: '崇高步兵容量 +500k',
      effects: [],
    },
    '19ae0569:node_25': {
      name: '骑兵扩编II',
      buff: '崇高骑兵容量 +500k',
      effects: [],
    },
    '19ae0569:node_26': {
      name: '弓兵扩编II',
      buff: '崇高弓兵容量 +500k',
      effects: [],
    },
    '19ae0569:node_27': {
      name: '利刃强化',
      buff: '攻击 +5%',
      effects: [],
    },
    '19ae0569:node_28': {
      name: '钢铁护盾',
      buff: '崇高步兵防御 +10%',
      effects: [],
    },
    '19ae0569:node_29': {
      name: '战马护甲',
      buff: '崇高骑兵防御 +10%',
      effects: [],
    },
    '19ae0569:node_30': {
      name: '护手',
      buff: '崇高弓兵防御 +10%',
      effects: [],
    },
    '19ae0569:node_31': {
      name: '钢剑',
      buff: '崇高步兵攻击 +10%',
      effects: [],
    },
    '19ae0569:node_32': {
      name: '锋利长枪',
      buff: '崇高骑兵攻击 +10%',
      effects: [],
    },
    '19ae0569:node_33': {
      name: '弩',
      buff: '崇高弓兵攻击 +10%',
      effects: [],
    },
    '19ae0569:node_34': {
      name: '守军步兵扩编',
      buff: '崇高步兵容量 +1m',
      effects: [],
    },
    '19ae0569:node_35': {
      name: '守军骑兵扩编',
      buff: '崇高骑兵容量 +1m',
      effects: [],
    },
    '19ae0569:node_36': {
      name: '守军弓兵扩编',
      buff: '崇高弓兵容量 +1m',
      effects: [],
    },
    '19ae0569:node_37': {
      name: '生命祝福',
      buff: '崇高HP +5%',
      effects: [],
    },
      '9c5d4006:node_1': {
      name: '防御之盾',
      buff: '步兵守城防御 +10%',
    },
    '9c5d4006:node_2': {
      name: '防御铠甲',
      buff: '弓兵守城防御 +10%',
    },
    '9c5d4006:node_3': {
      name: '防御骑兵',
      buff: '骑兵守城防御 +10%',
    },
    '9c5d4006:node_4': {
      name: '步兵突破',
      buff: '步兵守城攻击 +10%',
    },
    '9c5d4006:node_5': {
      name: '弓兵突破',
      buff: '弓兵守城攻击 +10%',
    },
    '9c5d4006:node_6': {
      name: '骑兵突破',
      buff: '骑兵守城攻击 +10%',
    },
    '9c5d4006:node_7': {
      name: '近战战术防护',
      buff: '全体单位战术防御 +5%',
    },
    '9c5d4006:node_8': {
      name: '坚毅 - 职业军团',
      buff: '全体单位职业军团防御 +15%',
    },
    '9c5d4006:node_9': {
      name: '坚毅 - 军团I',
      buff: '全体单位军团I防御 +15%',
    },
    '9c5d4006:node_10': {
      name: '坚毅 - 军团II',
      buff: '全体单位军团II防御 +15%',
    },
    '9c5d4006:node_11': {
      name: '坚毅 - 军团III',
      buff: '全体单位军团III防御 +15%',
    },
    '9c5d4006:node_12': {
      name: '坚毅 - 守城队列',
      buff: '全体单位守城队列防御 +15%',
    },
    '9c5d4006:node_13': {
      name: '坚固防御',
      buff: '全体单位守城防御 +10%',
    },
    '9c5d4006:node_14': {
      name: '防御部队',
      buff: '守城队列行军上限增加 15000',
    },
    '9c5d4006:node_15': {
      name: '近战战术打击',
      buff: '全体单位战术攻击 +5%',
    },
    '9c5d4006:node_16': {
      name: '坚毅 - 职业军团 M',
      buff: '全体单位职业军团攻击 +15%',
    },
    '9c5d4006:node_17': {
      name: '坚毅 - 军团I M',
      buff: '全体单位军团I攻击 +15%',
    },
    '9c5d4006:node_18': {
      name: '坚毅 - 军团II M',
      buff: '全体单位军团II攻击 +15%',
    },
    '9c5d4006:node_19': {
      name: '坚毅 - 军团III M',
      buff: '全体单位军团III攻击 +15%',
    },
    '9c5d4006:node_20': {
      name: '勇气 - 守城队列',
      buff: '全体单位守城队列攻击 +15%',
    },
    '9c5d4006:node_21': {
      name: '先机',
      buff: '全体单位守城攻击 +10%',
    },
    '9c5d4006:node_22': {
      name: '近战 - 步兵之盾',
      buff: '步兵防御 +15%',
    },
    '9c5d4006:node_23': {
      name: '近战 - 后方骑兵',
      buff: '骑兵防御 +15%',
    },
    '9c5d4006:node_24': {
      name: '近战 - 弓兵掩护',
      buff: '弓兵防御 +15%',
    },
    '9c5d4006:node_25': {
      name: '近战 - 步兵突袭',
      buff: '步兵攻击 +15%',
    },
    '9c5d4006:node_26': {
      name: '近战 - 骑兵突击',
      buff: '骑兵攻击 +15%',
    },
    '9c5d4006:node_27': {
      name: '近战 - 弓兵射击',
      buff: '弓兵攻击 +15%',
    },
    '9c5d4006:node_28': {
      name: '强健体魄',
      buff: '全体单位守城HP +10%',
    },
    '9c5d4006:node_29': {
      name: '近战生生不息',
      buff: '守城战斗中-5%，降低致命效果',
    },
    'b57d6bf9:node_1': {
      name: '利刃突击 - 步兵',
      buff: '步兵攻城攻击 +10%',
    },
    'b57d6bf9:node_2': {
      name: '利刃突击 - 骑兵',
      buff: '骑兵攻城攻击 +10%',
    },
    'b57d6bf9:node_3': {
      name: '利刃突击 - 弓兵',
      buff: '弓兵攻城攻击 +10%',
    },
    'b57d6bf9:node_4': {
      name: '利刃奇袭 - 步兵',
      buff: '步兵攻城防御 +10%',
    },
    'b57d6bf9:node_5': {
      name: '利刃奇袭 - 骑兵',
      buff: '骑兵攻城防御 +10%',
    },
    'b57d6bf9:node_6': {
      name: '利刃奇袭 - 弓兵',
      buff: '弓兵攻城防御 +10%',
    },
    'b57d6bf9:node_7': {
      name: '宫廷卫队',
      buff: '全体单位战术攻击 +5%',
    },
    'b57d6bf9:node_8': {
      name: '铁壁 - 职业军团',
      buff: '职业军团内全体单位防御 +10%',
    },
    'b57d6bf9:node_9': {
      name: '铁壁 - 军团1',
      buff: '军团1内全体单位防御 +10%',
    },
    'b57d6bf9:node_10': {
      name: '铁壁 - 军团2',
      buff: '军团2内全体单位防御 +10%',
    },
    'b57d6bf9:node_11': {
      name: '铁壁 - 军团3',
      buff: '军团3内全体单位防御 +10%',
    },
    'b57d6bf9:node_12': {
      name: '均衡战术',
      buff: '部队攻击和防御 +5%',
    },
    'b57d6bf9:node_13': {
      name: '突击 - 职业军团',
      buff: '职业军团内全体单位攻击 +10%',
    },
    'b57d6bf9:node_14': {
      name: '突击 - 军团1',
      buff: '军团1内全体单位攻击 +10%',
    },
    'b57d6bf9:node_15': {
      name: '突击 - 军团2',
      buff: '军团2内全体单位攻击 +10%',
    },
    'b57d6bf9:node_16': {
      name: '突击 - 军团3',
      buff: '军团3内全体单位攻击 +10%',
    },
    'b57d6bf9:node_17': {
      name: '先锋装甲',
      buff: '全体单位战术防御 +5%',
    },
    'b57d6bf9:node_18': {
      name: '重装先锋',
      buff: '全体单位攻城防御 +5%',
    },
    'b57d6bf9:node_19': {
      name: '战斗技能精通 - 步兵',
      buff: '步兵技能攻击 +10%',
    },
    'b57d6bf9:node_20': {
      name: '战斗技能精通 - 骑兵',
      buff: '骑兵技能攻击 +10%',
    },
    'b57d6bf9:node_21': {
      name: '战斗技能精通 - 弓兵',
      buff: '弓兵技能攻击 +10%',
    },
    'b57d6bf9:node_22': {
      name: '战斗技能抗性 - 步兵',
      buff: '步兵技能防御 +10%',
    },
    'b57d6bf9:node_23': {
      name: '战斗技能抗性 - 骑兵',
      buff: '骑兵技能防御 +10%',
    },
    'b57d6bf9:node_24': {
      name: '战斗技能抗性 - 弓兵',
      buff: '弓兵技能防御 +10%',
    },
    'b57d6bf9:node_25': {
      name: '先锋之魂 - 步兵',
      buff: '步兵HP提升 +5%',
    },
    'b57d6bf9:node_26': {
      name: '先锋之魂 - 骑兵',
      buff: '骑兵HP提升 +5%',
    },
    'b57d6bf9:node_27': {
      name: '先锋之魂 - 弓兵',
      buff: '弓兵HP提升 +5%',
    },
    'b57d6bf9:node_28': {
      name: '全面推进',
      buff: '全体士兵攻城攻击 +5%',
    },
    'b57d6bf9:node_29': {
      name: '阵型精通',
      buff: '攻城方HP +5%',
    },
    'b57d6bf9:node_30': {
      name: '铁甲洪流',
      buff: '造成伤害 +5%',
    },
},
};

export default researchContent;
