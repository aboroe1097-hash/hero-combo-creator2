...
// Pure and called for every name on every render, so results are memoised.
// The same few hundred names repeat; the cap only guards against unbounded
// growth from free-text searches.
const compactPlayerIdentityCache = new Map();
const COMPACT_IDENTITY_CACHE_LIMIT = 5000;

export function compactPlayerIdentity(name) {
  const input = String(name || '');
  const cached = compactPlayerIdentityCache.get(input);
  ...
  const result = input
    ...
    .replace(/[^\p{L}\p{N}]+/gu, '')
    .toLowerCase();
  if (compactPlayerIdentityCache.size >= COMPACT_IDENTITY_CACHE_LIMIT) {
    compactPlayerIdentityCache.clear();
  }
  compactPlayerIdentityCache.set(input, result);
  return result;
}
...
// Attack player lists are edited in place, so their caches are checked against
// a content signature. Building it is O(players), and name resolution asks once
// per player, which made every attack O(players²). Two opt-ins skip the check:
// lists the caller promises never change (published data, internal copies), and
// lists already verified inside the current explicit pass (withSignaturePass),
// during which the caller must not edit them.
const stableAttackLists = new WeakSet();
let signaturePassDepth = 0;
let signaturePassCounter = 0;
let activeSignaturePass = 0;

export function markAttackPlayersStable(list) {
  if (Array.isArray(list)) stableAttackLists.add(list);
  return list;
}

export function withSignaturePass(fn) {
  if (!signaturePassDepth) activeSignaturePass = ++signaturePassCounter;
  signaturePassDepth += 1;
  try {
    return fn();
  } finally {
    signaturePassDepth -= 1;
    if (!signaturePassDepth) activeSignaturePass = 0;
  }
}

export function currentSignaturePass() {
  return activeSignaturePass;
}

// True when a cached entry for this list can be trusted without rebuilding its
// signature.
export function attackListStillVerified(cached, list) {
  if (!cached) return false;
  if (stableAttackLists.has(list)) return true;
  return activeSignaturePass !== 0 && cached.pass === activeSignaturePass;
}
...
function buildAttackPlayerResolutionContext(attackPlayers) {
  ...
  if (sameInputs && attackListStillVerified(cached, attackPlayers)) return cached;
  const signature = attackPlayerResolutionSignature(attackPlayers);
  if (sameInputs && cached.signature === signature) {
    cached.pass = pass;
    return cached;
  }
...
// OCR spellings of known names. Built once: findBestMatch runs for every name
// in every attack, and rebuilding this table per call dominated page load.
const FIND_BEST_MATCH_ALIASES = Object.freeze({
  'كي미 kimmy': '키미 kimmy',
  'キミ kimmy': '키미 kimmy',
  'كيمي kimmy': '키미 kimmy',
  'кими kimmy': '키미 kimmy',
  ...
  MalakAdo: 'MalakAdo',
  MalakAbo: 'MalakAbo',
  'VICKED RUSSIAN': 'WICKED RUSSIAN',
  'Vicked Russian': 'WICKED RUSSIAN',
  'vicked Russian': 'WICKED RUSSIAN',
  'vicked russian': 'WICKED RUSSIAN',
  'Wicked Russian': 'WICKED RUSSIAN',
  'wicked russian': 'WICKED RUSSIAN',
  ...
  '«I N d ø»Made3110': 'Made3110',
  '♀️i N d ♦️/Made3110': 'Made3110',
  '♫ I N d ♦)Made3110': 'Made3110',
  '↣N d°/Made3110': 'Made3110',
  "'Isaan'": '°Isaan°',
  Тхий58: 'Тихий58',
  ОUNDEA: 'UNDEAD',
  ОUNDEAD: 'UNDEAD',
...
export function findBestMatch(name, minConfidence = 100) {
  ...
  if (typeof name === 'string') {
    ...
    const aliasTarget = Object.hasOwn(FIND_BEST_MATCH_ALIASES, name)
      ? FIND_BEST_MATCH_ALIASES[name]
      : '';
    if (aliasTarget) return resolveConfirmedPlayerAlias(aliasTarget) || aliasTarget;
