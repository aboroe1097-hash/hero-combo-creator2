...
function cleanEffectLabel(buff, token) {
  if (!buff) return 'Buff';
  const withoutToken = token ? buff.replace(token.raw, '').replace(/\bper\s+level\b/gi, '') : buff;
  return (
    withoutToken
      .replace(/\s+/g, ' ')
      .replace(/\s+([,.;:])/g, '$1')
...
export function getResearchBuffTone(label) {
  const normalized = String(label || '').toLowerCase();
  const isMight = /\b(?:might|attack|offen[cs]e)\b/.test(normalized);
  if (/\b(?:tactical|tac\.)\b/.test(normalized) && isMight) return 'tactical';
  if (/\b(?:resistance|resist|defen[cs]e|immunity)\b/.test(normalized)) return 'resistance';
  if (/\b(?:hp|health|healing|wounded)\b/.test(normalized)) return 'hp';
  if (/\b(?:damage|dmg|lethality)\b/.test(normalized)) return 'damage';
  if (isMight) return 'might';
  if (/\b(?:speed|haste|march)\b/.test(normalized)) return 'speed';
  if (/\b(?:income|gather|resource|production|capacity|load|cost)\b/.test(normalized)) {
    return 'economy';
  }
  return 'season';
}

const RESEARCH_PRIORITY_TIERS = Object.freeze([
  Object.freeze({ key: 'damage', rank: 1, label: 'Damage' }),
  Object.freeze({ key: 'hp', rank: 2, label: 'HP' }),
  Object.freeze({ key: 'tactical', rank: 3, label: 'Tactical Might / Resistance' }),
  Object.freeze({ key: 'might_resistance', rank: 4, label: 'Might / Resistance' }),
  Object.freeze({ key: 'training_speed', rank: 5, label: 'Training Speed' }),
  Object.freeze({ key: 'other', rank: 6, label: 'Other utility' }),
]);

/**
 * Stable combat-first research ordering used by both the Research UI and Velo.
 * The label is assembled from canonical node data; no value or effect is inferred.
 */
export function getResearchPriority(node = {}) {
  const label = `${node.name || ''} ${node.buff || ''} ${
    Array.isArray(node.buffStats)
      ? node.buffStats.map((stat) => stat?.label || '').join(' ')
      : node.buffStats?.label || node.buffStat?.label || ''
  }`.toLowerCase();
  const hasTactical = /\b(?:tactical|tac\.)\b/.test(label);
  const hasMight = /\b(?:might|attack|offen[cs]e)\b/.test(label);
  const hasResistance = /\b(?:resistance|resist|defen[cs]e|immunity)\b/.test(label);

  let key = 'other';
  if (/\b(?:damage|dmg|lethality)\b/.test(label)) key = 'damage';
  else if (/\b(?:hp|health)\b/.test(label)) key = 'hp';
  else if (hasTactical && (hasMight || hasResistance)) key = 'tactical';
  else if (hasMight || hasResistance) key = 'might_resistance';
  else if (/\btraining\s+(?:speed|haste)\b|\b(?:speed|haste)\s+training\b/.test(label)) {
    key = 'training_speed';
  }

  const tier = RESEARCH_PRIORITY_TIERS.find((candidate) => candidate.key === key);
  return {
    key: tier.key,
    rank: tier.rank,
    label: tier.label,
    basis:
      tier.key === 'other'
        ? 'No preferred combat or training-speed stat was identified in the canonical node label.'
        : `Canonical node data identifies ${tier.label.toLowerCase()}.`,
  };
}
...
function isLevelReference(buff, match) {
  ...
  const after = buff
    .slice(match.index + match[0].length, match.index + match[0].length + 6)
    .toLowerCase();
...
export function describeNodeBuffProgress(node, level = 0) {
  ...
  if (effects.length) {
    const effectRows = effects.map((effect) => {
      ...
      const levelValueIndex = hasLevelValues
        ? Math.min(safeLevel, effect.levelValues.length) - 1
        : -1;
      const currentValue =
        levelValueIndex >= 0
