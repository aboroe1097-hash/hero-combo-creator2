...
const ICON_SVGS = {
    ...
    // New icons added for v13.1.0 release
    forge: '<path d="M4 17h16v-2c0-1.5-.8-3-2-3H6c-1.2 0-2 1.5-2 3v2Z" stroke="currentColor" stroke-width="1.6" fill="currentColor" fill-opacity=".15"/><path d="M7 6h10v4c0 1.5-1.5 3-3.5 3h-3C8.5 13 7 11.5 7 10V6ZM10 17v-4h4v4M3 17h18" stroke="currentColor" stroke-width="1.6" stroke-linejoin="round"/>',
    tower: '<path d="M7 20h10M10 20v-5h4v5M9 15h6V9H9v6ZM8 9h8L15 5H9L8 9ZM12 2v3" stroke="currentColor" stroke-width="1.6" stroke-linejoin="round" fill="currentColor" fill-opacity=".1"/>',
    target: '<circle cx="12" cy="12" r="7.5" stroke="currentColor" stroke-width="1.6" fill="currentColor" fill-opacity=".1"/><circle cx="12" cy="12" r="1.5" fill="currentColor"/><path d="M12 2v3M12 19v3M2 12h3M19 12h3" stroke="currentColor" stroke-width="1.6" stroke-linecap="round"/>',
    hp: '<path d="M12 6.5C10.5 4 6.5 4 5.2 6.5c-1.5 3 1.3 6.5 6.8 11 5.5-4.5 8.3-8 6.8-11C17.5 4 13.5 4 12 6.5Z" stroke="currentColor" stroke-width="1.6" fill="currentColor" fill-opacity=".15" stroke-linejoin="round"/>',
    medal: '<path d="M9 4h6l-1 5H10L9 4Z" stroke="currentColor" stroke-width="1.6" stroke-linejoin="round"/><circle cx="12" cy="14" r="5" stroke="currentColor" stroke-width="1.6" fill="currentColor" fill-opacity=".15"/><path d="M12 12v4M10 14h4" stroke="currentColor" stroke-width="1.4" stroke-linecap="round"/>',
...
const MATCH_RULES = [
    ...
    { icon: 'forge', score: 113, test: ({ text }) => /refining|processing|furnace|forge|forging|salvage|salvaging|disenchant|crafting|shoemaker|shoe maker|armorer|jeweler|trinket|equipment/i.test(text) },
    { icon: 'hospital', score: 112, test: ({ text }) => /hospital|sanctuary|medical|medicine|clinic|meds|safe haven/i.test(text) },
    { icon: 'bed', score: 111, test: ({ text }) => /temp bed|wounded capacity|overflow wounded/i.test(text) },
    { icon: 'heal', score: 110, test: ({ text }) => /first-?aid|healing|treatment|veterinarian|super healing|battlefield first aid|life goes on/i.test(text) },
    { icon: 'tower', score: 109, test: ({ text }) => /tower\b|fortification/i.test(text) },
    { icon: 'target', score: 108, test: ({ text }) => /\bmark\b|battle scar|focus fire|target weakness|deadly aim/i.test(text) },
    { icon: 'rebel', score: 108, test: ({ text }) => /rebel slayer|marauder|for the people/i.test(text) },
    { icon: 'defender', score: 107, test: ({ text }) => /empire defender|siege defense|defenders of peace|defense|peace|building walls/i.test(text) },
    { icon: 'siege', score: 106, test: ({ text }) => /siege|claymore|arrow hail|single player siege|paralysis|destruction|demolition|durability/i.test(text) },
    { icon: 'death', score: 105, test: ({ text }) => /deadly|death rate|death ratio|slaughter/i.test(text) },
    { icon: 'hp', score: 104, test: ({ text }) => /\bhp\b|\bhealth\b|spring of life|morale|tenacity|blessing of life|carbon fiber bones|viral spores/i.test(text) },
    ...
    { icon: 'medal', score: 102, test: ({ text }) => /double reward|\bpoints\b|\bscore\b|incentive|inspire|arms expert|double chest/i.test(text) },
    ...
    { icon: 'march', score: 98, test: ({ text }) => /march(?:ing)?\s+speed|horseshoe|purebred|stand your ground|ferocious|fervious|charge|onslaught/i.test(text) },
    { icon: 'load', score: 97, test: ({ text }) => /march load|transport cart|enhanced physique|\bload\b/i.test(text) },
    { icon: 'gather', score: 96, test: ({ text }) => /gather(?:ing)?|effective mining|distribution/i.test(text) },
    ...
    { icon: 'stability', score: 84, test: ({ text }) => /stability|will and way|security/i.test(text) },
    { icon: 'recruit', score: 83, test: ({ text }) => /recruitment|conscription|blessed soldiers/i.test(text) },
    { icon: 'training', score: 82, test: ({ text }) => /training|capacity/i.test(text) },
    { icon: 'assault', score: 81, test: ({ text }) => /\bassault\b|power strike|deadly aim|focus fire|hunt\b|brutal attack|payback|attack|strike|advance/i.test(text) },
    { icon: 'sword', score: 80, test: ({ text }) => /claymore|deadly arms|poisoned|enhanced arrow|target weakness/i.test(text) },
    { icon: 'shield', score: 79, test: ({ text }) => /shield|stand your ground|last stand/i.test(text) },
    { icon: 'might', score: 78, test: ({ text }) => /\bmight\b|power strike|power|hunger for battle/i.test(text) },
    { icon: 'resistance', score: 77, test: ({ text }) => /\bresistance\b|immunity|mastercraft armour|mitigation|toughness/i.test(text) },
