...
import { buildCompetitionGrowthRows, rankCompetitionGrowth } from './competition-growth.js';
...
/**
 * Competition #12 read-only 2026 growth preview. The live public board is
 * generated by the vtsScore function from all earlier seasons.
 */
export function createCompetitionGrowthSection(options = {}) {
  ...
  function paint() {
    ...
    if (!state.snapshot) {
      ...
      const { ranked, notRanked } = rankCompetitionGrowth(rowsFor());
      const all = [...ranked, ...notRanked];
      const pending = all.filter((row) => row.match.status === 'ambiguous').length;
      const heads = [
        'adminThRank',
        'adminVtsScorePlayer',
    ...
    host.innerHTML = `<div class="vts-admin-card-heading"><div><h3>${esc(t('c12Title'))}</h3><p>${esc(t('c12Hint'))}</p></div><div class="vts-admin-chip-row"><button type="button" class="dash-btn" data-comp12-reload>${esc(t('adminVtsScoreRefresh'))}</button></div></div>${body}`;
    bind();
  }
