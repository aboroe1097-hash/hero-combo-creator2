...
test.describe('Eden Siege', () => {
  ...
  test('a terminal phase stops rendering until restart', async ({ page }) => {
    ...
    await expect
      ...
      .toBe('ready');
    // The restart schedules the next frame; wait for it rather than racing it.
    await expect
      .poll(() => page.evaluate(() => window.__EDEN_SIEGE__.game.stats().frames))
      .toBeGreaterThan(terminalStats.frames);
    await expect
      .poll(() => page.evaluate(() => window.__EDEN_SIEGE__.game.stats().simSteps))
