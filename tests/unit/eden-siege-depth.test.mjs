...
import { dailySiegeFor as arcadeDailySiegeFor } from '../../js/siege-daily.js';
import {
  BOSS,
  DASH,
  MODIFIER_ORDER,
...
test('an idle player cannot lose the stronghold during training', () => {
  const world = createWorld({ mapId: 'keep', seed: 'keep:tutorial-idle', tutorial: true });
  world.setInput({ start: true });
  world.step();
  assert.equal(world.state.tutorial.active, true);
  // Nobody defends: the rangers walk up and hit the stronghold for minutes.
  advance(world, SECOND * 420);
  assert.equal(world.state.tutorial.active, true, 'training waits for the player');
  assert.notEqual(world.state.phase, 'defeat');
  assert.ok(world.state.core.hp < world.state.core.maxHp, 'the rangers did reach the stronghold');
  assert.ok(world.state.core.hp >= world.state.core.maxHp * 0.5 - 1e-9, 'but only dent it');
});
