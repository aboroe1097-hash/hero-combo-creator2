...
test('Skins view reserves item icon slots and styles the gallery for both themes', () => {
  ...
  assert.match(detailCss, /\[data-theme='light'\] \.skin-gallery-card \{[\s\S]*?linear-gradient\(180deg, rgba\(255, 255, 255, 0\.98\)/);
  assert.match(detailCss, /\[data-theme='light'\] \.skin-item-icon \{/);
  assert.match(detailCss, /@media \(max-width: 768px\) \{[\s\S]*?\.skin-gallery-grid \{[\s\S]*?repeat\(2, minmax\(0, 1fr\)\)/);
  assert.match(detailCss, /@media \(prefers-reduced-motion: reduce\) \{[\s\S]*?\.skin-gallery-card/);
